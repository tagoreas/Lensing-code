...
/*********************************************************/
/*             CODE MODIFICATION NOTICE!!!!              */
/*                                                       */
/* This Triangle source code has been modified.          */
/* See the section immediately below the following       */
/* copyright information for details                     */
/*********************************************************/
...
/*   CODE MODIFICATION NOTICE!!!                                             */
/*                                                                           */
/* select integer types throughout this code have been changed to allow for  */
/* 32- or 64-bit integers to be used.                                        */
/* The original code used the 'int' data type to index points and triangles. */
/* I have now allowed a compile-time flaf that will change these to          */
/* 'long int'                                                                */
/*****************************************************************************/
...
#ifndef TRILIBRARY
char *ps_tri_readline();
char *ps_tri_findfield();
...
struct otri {
    triangle *tri;
    PS_SIT orient;                                         /* Ranges from 0 to 2. */
};
...
struct osub {
    subseg *ss;
    PS_SIT ssorient;                                       /* Ranges from 0 to 1. */
};
...
struct event {
    ...
    PS_SIT heapposition;              /* Marks this event's position in the heap. */
};
...
struct memorypool {
    ...
    PS_SIT alignbytes;
    PS_SIT itembytes;
    PS_SIT itemsperblock;
    PS_SIT itemsfirstblock;
    long long items, maxitems;
    PS_SIT unallocateditems;
    PS_SIT pathitemsleft;
};


/* Global constants.                                                         */

REAL ps_tri_splitter;       /* Used to split REAL factors for exact multiplication. */
REAL ps_tri_epsilon;                             /* Floating-point machine epsilon. */
REAL ps_tri_resulterrbound;
REAL ps_tri_ccwerrboundA, ps_tri_ccwerrboundB, ps_tri_ccwerrboundC;
REAL ps_tri_iccerrboundA, ps_tri_iccerrboundB, ps_tri_iccerrboundC;
REAL ps_tri_o3derrboundA, ps_tri_o3derrboundB, ps_tri_o3derrboundC;

/* Random number seed is not constant, but I've made it global anyway.       */

unsigned long long ps_tri_randomseed;                     /* Current random number seed. */
...
/*   ordered from 4095 (highest priority) to 0 (lowest priority).            */
    ...
    PS_SIT nextnonemptyq[4096];
    PS_SIT firstnonemptyq;
...
/* Other variables. */
    ...
    PS_SIT invertices;                               /* Number of input vertices. */
    PS_SIT inelements;                              /* Number of input triangles. */
    PS_SIT insegments;                               /* Number of input segments. */
    PS_SIT holes;                                       /* Number of input holes. */
    PS_SIT regions;                                   /* Number of input regions. */
    PS_SIT undeads;    /* Number of input vertices that don't appear in the mesh. */
    long long edges;                                     /* Number of output edges. */
    PS_SIT mesh_dim;                                /* Dimension (ought to be 2). */
    PS_SIT nextras;                           /* Number of attributes per vertex. */
    PS_SIT eextras;                         /* Number of attributes per triangle. */
    long long hullsize;                          /* Number of edges in convex hull. */
    PS_SIT steinerleft;                 /* Number of Steiner points not yet used. */
    PS_SIT vertexmarkindex;         /* Index to find boundary marker of a vertex. */
    PS_SIT vertex2triindex;     /* Index to find a triangle adjacent to a vertex. */
    PS_SIT highorderindex;  /* Index to find extra nodes for high-order elements. */
    PS_SIT elemattribindex;            /* Index to find attributes of a triangle. */
    PS_SIT areaboundindex;             /* Index to find area bound of a triangle. */
    PS_SIT checksegments;         /* Are there segments in the triangulation yet? */
    PS_SIT checkquality;                  /* Has quality triangulation begun yet? */
    PS_SIT readnodefile;                           /* Has a .node file been read? */
...
/* Read the instructions to find out the meaning of these switches.          */
    ...
    PS_SIT poly, refine, quality, vararea, fixedarea, usertest;
    PS_SIT regionattrib, convex, weighted, jettison;
    PS_SIT firstnumber;
    PS_SIT edgesout, voronoi, neighbors, geomview;
    PS_SIT nobound, nopolywritten, nonodewritten, noelewritten, noiterationnum;
    PS_SIT noholes, noexact, conformdel;
    PS_SIT incremental, sweepline, dwyer;
    PS_SIT splitseg;
    PS_SIT docheck;
    PS_SIT quiet, verbose;
    PS_SIT usesegments;
    PS_SIT order;
    PS_SIT nobisect;
    PS_SIT steiner;
...
PS_SIT ps_tri_plus1mod3[3] = {1, 2, 0};
PS_SIT ps_tri_minus1mod3[3] = {2, 0, 1};
...
#define decode(ptr, otri)                                               \
    (otri).orient = (PS_SIT) ((unsigned long long) (ptr) & (unsigned long long) 3l); \
          (otri).tri = (triangle *)                                     \
...
#define lnext(otri1, otri2)                                     \
    (otri2).tri = (otri1).tri;                                  \
           (otri2).orient = ps_tri_plus1mod3[(otri1).orient]

#define lnextself(otri)                                 \
    (otri).orient = ps_tri_plus1mod3[(otri).orient]

/* lprev() finds the previous edge (clockwise) of a triangle.                */

#define lprev(otri1, otri2)                                     \
    (otri2).tri = (otri1).tri;                                  \
           (otri2).orient = ps_tri_minus1mod3[(otri1).orient]

#define lprevself(otri)                                 \
    (otri).orient = ps_tri_minus1mod3[(otri).orient]
...
#define org(otri, vertexptr)                                            \
    vertexptr = (vertex) (otri).tri[ps_tri_plus1mod3[(otri).orient] + 3]

#define dest(otri, vertexptr)                                           \
    vertexptr = (vertex) (otri).tri[ps_tri_minus1mod3[(otri).orient] + 3]

#define apex(otri, vertexptr)                           \
    vertexptr = (vertex) (otri).tri[(otri).orient + 3]

#define setorg(otri, vertexptr)                                         \
    (otri).tri[ps_tri_plus1mod3[(otri).orient] + 3] = (triangle) vertexptr

#define setdest(otri, vertexptr)                                        \
    (otri).tri[ps_tri_minus1mod3[(otri).orient] + 3] = (triangle) vertexptr
...
#define sdecode(sptr, osub)                                             \
    (osub).ssorient = (PS_SIT) ((unsigned long long) (sptr) & (unsigned long long) 1l); \
          (osub).ss = (subseg *)                                        \
...
#define mark(osub)  (* (PS_SIT *) ((osub).ss + 8))

#define setmark(osub, value)                    \
    * (PS_SIT *) ((osub).ss + 8) = value
...
#define vertexmark(vx)  ((PS_SIT *) (vx))[m->vertexmarkindex]

#define setvertexmark(vx, value)                        \
    ((PS_SIT *) (vx))[m->vertexmarkindex] = value

#define vertextype(vx)  ((PS_SIT *) (vx))[m->vertexmarkindex + 1]

#define setvertextype(vx, value)                        \
    ((PS_SIT *) (vx))[m->vertexmarkindex + 1] = value
...
PS_SIT ps_tri_triunsuitable();

#else /* not EXTERNAL_TEST */

#ifdef ANSI_DECLARATORS
PS_SIT ps_tri_triunsuitable(vertex triorg, vertex tridest, vertex triapex, REAL area)
#else /* not ANSI_DECLARATORS */
    PS_SIT ps_tri_triunsuitable(triorg, tridest, triapex, area)
...
#ifdef ANSI_DECLARATORS
void ps_tri_triexit(PS_SIT status)
#else /* not ANSI_DECLARATORS */
    void ps_tri_triexit(status)
    PS_SIT status;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
VOID *ps_tri_trimalloc(PS_SIT size)
#else /* not ANSI_DECLARATORS */
VOID *ps_tri_trimalloc(size)
PS_SIT size;
#endif /* not ANSI_DECLARATORS */

{
    ...
    memptr = (VOID *) malloc((PS_unsignedSIT) size);
    if (memptr == (VOID *) NULL) {
...
#ifdef ANSI_DECLARATORS
void ps_tri_trifree(VOID *memptr)
#else /* not ANSI_DECLARATORS */
    void ps_tri_trifree(memptr)
    VOID *memptr;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_parsecommandline(PS_SIT argc, char **argv, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_parsecommandline(argc, argv, b)
    PS_SIT argc;
    char **argv;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */
        ...
        if (argv[i][0] == '-') {
            ...
            for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
                ...
                if (argv[i][j] == 'S') {
                    ...
                    while ((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) {
                        j++;
                        b->steiner = b->steiner * 10 + (PS_SIT) (argv[i][j] - '0');
                    }
                }
#endif /* not CDT_ONLY */
#ifndef REDUCED
    ...
    if (increment > 0) {
        ...
        do {
            if ((workstring[j] >= '0') && (workstring[j] <= '9')) {
                meshnumber = meshnumber * 10 + (PS_SIT) (workstring[j] - '0');
            } else {
                increment = 0;
            }
            j++;
...
#ifdef ANSI_DECLARATORS
void ps_tri_printtriangle(struct mesh *m, struct behavior *b, struct otri *t)
#else /* not ANSI_DECLARATORS */
    void ps_tri_printtriangle(m, b, t)
    struct mesh *m;
    struct behavior *b;
    struct otri *t;
#endif /* not ANSI_DECLARATORS */

{
    ...
    if (printvertex == (vertex) NULL)
        printf("    Origin[%lld] = NULL\n", ((long long)t->orient + 1) % 3 + 3);
    else
        printf("    Origin[%lld] = x%llx  (%.12g, %.12g)\n",
               ((long long)t->orient + 1) % 3 + 3, (unsigned long long) printvertex,
               printvertex[0], printvertex[1]);
    dest(*t, printvertex);
    if (printvertex == (vertex) NULL)
        printf("    Dest  [%lld] = NULL\n", ((long long)t->orient + 2) % 3 + 3);
    else
        printf("    Dest  [%lld] = x%llx  (%.12g, %.12g)\n",
               ((long long)t->orient + 2) % 3 + 3, (unsigned long long) printvertex,
               printvertex[0], printvertex[1]);
    apex(*t, printvertex);
    if (printvertex == (vertex) NULL)
        printf("    Apex  [%lld] = NULL\n", (long long)t->orient + 3);
    else
        printf("    Apex  [%lld] = x%llx  (%.12g, %.12g)\n",
               (long long)t->orient + 3, (unsigned long long) printvertex,
               printvertex[0], printvertex[1]);
...
#ifdef ANSI_DECLARATORS
void ps_tri_printsubseg(struct mesh *m, struct behavior *b, struct osub *s)
#else /* not ANSI_DECLARATORS */
    void ps_tri_printsubseg(m, b, s)
    struct mesh *m;
    struct behavior *b;
    struct osub *s;
#endif /* not ANSI_DECLARATORS */

{
    ...
    printf("subsegment x%llx with orientation %lld and mark %lld:\n",
           (unsigned long long) s->ss, (long long)s->ssorient, (long long)mark(*s));
    ...
    sorg(*s, printvertex);
    if (printvertex == (vertex) NULL)
        printf("    Origin[%lld] = NULL\n", 2 + (long long)s->ssorient);
    else
        printf("    Origin[%lld] = x%llx  (%.12g, %.12g)\n",
               2 + (long long)s->ssorient, (unsigned long long) printvertex,
               printvertex[0], printvertex[1]);
    sdest(*s, printvertex);
    if (printvertex == (vertex) NULL)
        printf("    Dest  [%lld] = NULL\n", 3 - (long long)s->ssorient);
    else
        printf("    Dest  [%lld] = x%llx  (%.12g, %.12g)\n",
               3 - (long long)s->ssorient, (unsigned long long) printvertex,
               printvertex[0], printvertex[1]);
    ...
    if (printvertex == (vertex) NULL)
        printf("    Segment origin[%lld] = NULL\n", 4 + (long long)s->ssorient);
    else
        printf("    Segment origin[%lld] = x%llx  (%.12g, %.12g)\n",
               4 + (long long)s->ssorient, (unsigned long long) printvertex,
               printvertex[0], printvertex[1]);
    segdest(*s, printvertex);
    if (printvertex == (vertex) NULL)
        printf("    Segment dest  [%lld] = NULL\n", 5 - (long long)s->ssorient);
    else
        printf("    Segment dest  [%lld] = x%llx  (%.12g, %.12g)\n",
               5 - (long long)s->ssorient, (unsigned long long) printvertex,
               printvertex[0], printvertex[1]);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_poolzero(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
    void ps_tri_poolzero(pool)
...
#ifdef ANSI_DECLARATORS
void ps_tri_poolrestart(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
    void ps_tri_poolrestart(pool)
    struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_poolinit(struct memorypool *pool, PS_SIT bytecount, PS_SIT itemcount,
                     PS_SIT firstitemcount, PS_SIT alignment)
#else /* not ANSI_DECLARATORS */
    void ps_tri_poolinit(pool, bytecount, itemcount, firstitemcount, alignment)
    struct memorypool *pool;
    PS_SIT bytecount;
    PS_SIT itemcount;
    PS_SIT firstitemcount;
    PS_SIT alignment;
#endif /* not ANSI_DECLARATORS */

{
    ...
    pool->firstblock = (VOID **)
        ps_tri_trimalloc(pool->itemsfirstblock * pool->itembytes + (PS_SIT) sizeof(VOID *) +
                         pool->alignbytes);
    /* Set the next block pointer to NULL. */
    *(pool->firstblock) = (VOID *) NULL;
    ps_tri_poolrestart(pool);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_pooldeinit(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
    void ps_tri_pooldeinit(pool)
    struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */

{
    while (pool->firstblock != (VOID **) NULL) {
        ...
        ps_tri_trifree((VOID *) pool->firstblock);
...
#ifdef ANSI_DECLARATORS
VOID *ps_tri_poolalloc(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
VOID *ps_tri_poolalloc(pool)
struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */
    ...
    if (pool->deaditemstack != (VOID *) NULL) {
        ...
        if (pool->unallocateditems == 0) {
            ...
            if (*(pool->nowblock) == (VOID *) NULL) {
                ...
                newblock = (VOID **) ps_tri_trimalloc(pool->itemsperblock * pool->itembytes +
                                                      (PS_SIT) sizeof(VOID *) +
                                                      pool->alignbytes);
...
#ifdef ANSI_DECLARATORS
void ps_tri_pooldealloc(struct memorypool *pool, VOID *dyingitem)
#else /* not ANSI_DECLARATORS */
    void ps_tri_pooldealloc(pool, dyingitem)
    struct memorypool *pool;
    VOID *dyingitem;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_traversalinit(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
    void ps_tri_traversalinit(pool)
    struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
VOID *ps_tri_traverse(struct memorypool *pool)
#else /* not ANSI_DECLARATORS */
VOID *ps_tri_traverse(pool)
struct memorypool *pool;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_dummyinit(struct mesh *m, struct behavior *b, PS_SIT trianglebytes,
                      PS_SIT subsegbytes)
#else /* not ANSI_DECLARATORS */
    void ps_tri_dummyinit(m, b, trianglebytes, subsegbytes)
    struct mesh *m;
    struct behavior *b;
    PS_SIT trianglebytes;
    PS_SIT subsegbytes;
#endif /* not ANSI_DECLARATORS */

{
    ...
    m->dummytribase = (triangle *) ps_tri_trimalloc(trianglebytes +
    ...
    if (b->usesegments) {
        ...
        /*   subsegment.                                                   */
        m->dummysubbase = (subseg *) ps_tri_trimalloc(subsegbytes +
...
#ifdef ANSI_DECLARATORS
void ps_tri_initializevertexpool(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_initializevertexpool(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    /*   a sizeof(PS_SIT)-byte address.                                          */
    m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(REAL) +
                          sizeof(PS_SIT) - 1) /
        sizeof(PS_SIT);
    vertexsize = (m->vertexmarkindex + 2) * sizeof(PS_SIT);
    if (b->poly) {
    ...
    ps_tri_poolinit(&m->vertices, vertexsize, VERTEXPERBLOCK,
...
#ifdef ANSI_DECLARATORS
void ps_tri_initializetrisubpools(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_initializetrisubpools(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    if ((b->voronoi || b->neighbors) &&
        (trisize < 6 * sizeof(triangle) + sizeof(PS_SIT))) {
        trisize = 6 * sizeof(triangle) + sizeof(PS_SIT);
    }

    /* Having determined the memory size of a triangle, initialize the pool. */
    ps_tri_poolinit(&m->triangles, trisize, TRIPERBLOCK,
    ...
    if (b->usesegments) {
        ...
        ps_tri_poolinit(&m->subsegs, 8 * sizeof(triangle) + sizeof(PS_SIT),
                        SUBSEGPERBLOCK, SUBSEGPERBLOCK, 8);

        /* Initialize the "outer space" triangle and omnipresent subsegment. */
        ps_tri_dummyinit(m, b, m->triangles.itembytes, m->subsegs.itembytes);
    } else {
        /* Initialize the "outer space" triangle. */
        ps_tri_dummyinit(m, b, m->triangles.itembytes, 0);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_triangledealloc(struct mesh *m, triangle *dyingtriangle)
#else /* not ANSI_DECLARATORS */
    void ps_tri_triangledealloc(m, dyingtriangle)
    struct mesh *m;
...

{
    ...
    ps_tri_pooldealloc(&m->triangles, (VOID *) dyingtriangle);
}
...
#ifdef ANSI_DECLARATORS
triangle *ps_tri_triangletraverse(struct mesh *m)
#else /* not ANSI_DECLARATORS */
triangle *ps_tri_triangletraverse(m)
struct mesh *m;
#endif /* not ANSI_DECLARATORS */

{
    ...
    do {
        newtriangle = (triangle *) ps_tri_traverse(&m->triangles);
...
#ifdef ANSI_DECLARATORS
void ps_tri_subsegdealloc(struct mesh *m, subseg *dyingsubseg)
#else /* not ANSI_DECLARATORS */
    void ps_tri_subsegdealloc(m, dyingsubseg)
    struct mesh *m;
    subseg *dyingsubseg;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_pooldealloc(&m->subsegs, (VOID *) dyingsubseg);
}
...
#ifdef ANSI_DECLARATORS
subseg *ps_tri_subsegtraverse(struct mesh *m)
#else /* not ANSI_DECLARATORS */
subseg *ps_tri_subsegtraverse(m)
struct mesh *m;
#endif /* not ANSI_DECLARATORS */

{
    ...
    do {
        newsubseg = (subseg *) ps_tri_traverse(&m->subsegs);
...
#ifdef ANSI_DECLARATORS
void ps_tri_vertexdealloc(struct mesh *m, vertex dyingvertex)
#else /* not ANSI_DECLARATORS */
    void ps_tri_vertexdealloc(m, dyingvertex)
    struct mesh *m;
    vertex dyingvertex;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_pooldealloc(&m->vertices, (VOID *) dyingvertex);
}
...
#ifdef ANSI_DECLARATORS
vertex ps_tri_vertextraverse(struct mesh *m)
#else /* not ANSI_DECLARATORS */
    vertex ps_tri_vertextraverse(m)
    struct mesh *m;
#endif /* not ANSI_DECLARATORS */

{
    ...
    do {
        newvertex = (vertex) ps_tri_traverse(&m->vertices);
...
#ifdef ANSI_DECLARATORS
void ps_tri_badsubsegdealloc(struct mesh *m, struct badsubseg *dyingseg)
#else /* not ANSI_DECLARATORS */
    void ps_tri_badsubsegdealloc(m, dyingseg)
    struct mesh *m;
...

{
    ...
    ps_tri_pooldealloc(&m->badsubsegs, (VOID *) dyingseg);
}
...
#ifdef ANSI_DECLARATORS
struct badsubseg *ps_tri_badsubsegtraverse(struct mesh *m)
#else /* not ANSI_DECLARATORS */
    struct badsubseg *ps_tri_badsubsegtraverse(m)
    struct mesh *m;
#endif /* not ANSI_DECLARATORS */

{
    ...
    do {
        newseg = (struct badsubseg *) ps_tri_traverse(&m->badsubsegs);
...
#ifdef ANSI_DECLARATORS
vertex ps_tri_getvertex(struct mesh *m, struct behavior *b, PS_SIT number)
#else /* not ANSI_DECLARATORS */
    vertex ps_tri_getvertex(m, b, number)
    struct mesh *m;
    struct behavior *b;
    PS_SIT number;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_triangledeinit(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_triangledeinit(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ps_tri_pooldeinit(&m->triangles);
    ps_tri_trifree((VOID *) m->dummytribase);
    if (b->usesegments) {
        ps_tri_pooldeinit(&m->subsegs);
        ps_tri_trifree((VOID *) m->dummysubbase);
    }
    ps_tri_pooldeinit(&m->vertices);
#ifndef CDT_ONLY
    if (b->quality) {
        ps_tri_pooldeinit(&m->badsubsegs);
        if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
            ps_tri_pooldeinit(&m->badtriangles);
            ps_tri_pooldeinit(&m->flipstackers);
        }
    }
#endif /* not CDT_ONLY */
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_maketriangle(struct mesh *m, struct behavior *b, struct otri *newotri)
#else /* not ANSI_DECLARATORS */
    void ps_tri_maketriangle(m, b, newotri)
    struct mesh *m;
    struct behavior *b;
    struct otri *newotri;
#endif /* not ANSI_DECLARATORS */

{
    ...
    newotri->tri = (triangle *) ps_tri_poolalloc(&m->triangles);
...
#ifdef ANSI_DECLARATORS
void ps_tri_makesubseg(struct mesh *m, struct osub *newsubseg)
#else /* not ANSI_DECLARATORS */
    void ps_tri_makesubseg(m, newsubseg)
    struct mesh *m;
    struct osub *newsubseg;
#endif /* not ANSI_DECLARATORS */

{
    newsubseg->ss = (subseg *) ps_tri_poolalloc(&m->subsegs);
    /* Initialize the two adjoining subsegments to be the omnipresent */
    /*   subsegment.                                                  */
...
void ps_tri_exactinit()
{
    ...
    do {
        ...
        check = 1.0 + ps_tri_epsilon;
    } while ((check != 1.0) && (check != lastcheck));
    ps_tri_splitter += 1.0;
    /* Error bounds for orientation and incircle tests. */
    ps_tri_resulterrbound = (3.0 + 8.0 * ps_tri_epsilon) * ps_tri_epsilon;
    ps_tri_ccwerrboundA = (3.0 + 16.0 * ps_tri_epsilon) * ps_tri_epsilon;
    ps_tri_ccwerrboundB = (2.0 + 12.0 * ps_tri_epsilon) * ps_tri_epsilon;
    ps_tri_ccwerrboundC = (9.0 + 64.0 * ps_tri_epsilon) * ps_tri_epsilon * ps_tri_epsilon;
    ps_tri_iccerrboundA = (10.0 + 96.0 * ps_tri_epsilon) * ps_tri_epsilon;
    ps_tri_iccerrboundB = (4.0 + 48.0 * ps_tri_epsilon) * ps_tri_epsilon;
    ps_tri_iccerrboundC = (44.0 + 576.0 * ps_tri_epsilon) * ps_tri_epsilon * ps_tri_epsilon;
    ps_tri_o3derrboundA = (7.0 + 56.0 * ps_tri_epsilon) * ps_tri_epsilon;
    ps_tri_o3derrboundB = (3.0 + 28.0 * ps_tri_epsilon) * ps_tri_epsilon;
    ps_tri_o3derrboundC = (26.0 + 288.0 * ps_tri_epsilon) * ps_tri_epsilon * ps_tri_epsilon;
}
...
#ifdef ANSI_DECLARATORS
PS_SIT ps_tri_fast_expansion_sum_zeroelim(PS_SIT elen, REAL *e, PS_SIT flen, REAL *f, REAL *h)
#else /* not ANSI_DECLARATORS */
    PS_SIT ps_tri_fast_expansion_sum_zeroelim(elen, e, flen, f, h)  /* h cannot be e or f. */
    PS_SIT elen;
    REAL *e;
    PS_SIT flen;
    REAL *f;
...
#ifdef ANSI_DECLARATORS
PS_SIT ps_tri_scale_expansion_zeroelim(PS_SIT elen, REAL *e, REAL b, REAL *h)
#else /* not ANSI_DECLARATORS */
    PS_SIT ps_tri_scale_expansion_zeroelim(elen, e, b, h)   /* e and h cannot be the same. */
    PS_SIT elen;
    REAL *e;
    REAL b;
    REAL *h;
...
#ifdef ANSI_DECLARATORS
REAL ps_tri_estimate(PS_SIT elen, REAL *e)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_estimate(elen, e)
    PS_SIT elen;
    REAL *e;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
REAL ps_tri_counterclockwiseadapt(vertex pa, vertex pb, vertex pc, REAL detsum)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_counterclockwiseadapt(pa, pb, pc, detsum)
    vertex pa;
    vertex pb;
    vertex pc;
    REAL detsum;
...

{
    ...
    INEXACT REAL B3;
    PS_SIT C1length, C2length, Dlength;
    REAL u[4];
    INEXACT REAL u3;
    INEXACT REAL s1, t1;
    REAL s0, t0;
    ...
    det = ps_tri_estimate(4, B);
    errbound = ps_tri_ccwerrboundB * detsum;
    ...
    errbound = ps_tri_ccwerrboundC * detsum + ps_tri_resulterrbound * Absolute(det);
    ...
    u[3] = u3;
    C1length = ps_tri_fast_expansion_sum_zeroelim(4, B, 4, u, C1);

    Two_Product(acx, bcytail, s1, s0);
    Two_Product(acy, bcxtail, t1, t0);
    Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
    u[3] = u3;
    C2length = ps_tri_fast_expansion_sum_zeroelim(C1length, C1, 4, u, C2);

    Two_Product(acxtail, bcytail, s1, s0);
    Two_Product(acytail, bcxtail, t1, t0);
    Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
    u[3] = u3;
    Dlength = ps_tri_fast_expansion_sum_zeroelim(C2length, C2, 4, u, D);

    return(D[Dlength - 1]);
}

#ifdef ANSI_DECLARATORS
REAL ps_tri_counterclockwise(struct mesh *m, struct behavior *b,
                             vertex pa, vertex pb, vertex pc)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_counterclockwise(m, b, pa, pb, pc)
    struct mesh *m;
    struct behavior *b;
    vertex pa;
    vertex pb;
...

{
    ...
    errbound = ps_tri_ccwerrboundA * detsum;
    if ((det >= errbound) || (-det >= errbound)) {
        return det;
    }

    return ps_tri_counterclockwiseadapt(pa, pb, pc, detsum);
}
...
#ifdef ANSI_DECLARATORS
REAL ps_tri_incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, REAL permanent)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_incircleadapt(pa, pb, pc, pd, permanent)
    vertex pa;
    vertex pb;
    vertex pc;
    vertex pd;
...

{
    ...
    PS_SIT axbclen, axxbclen, aybclen, ayybclen, alen;
    REAL bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
    PS_SIT bxcalen, bxxcalen, bycalen, byycalen, blen;
    REAL cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
    PS_SIT cxablen, cxxablen, cyablen, cyyablen, clen;
    REAL abdet[64];
    PS_SIT ablen;
    ...
    PS_SIT temp8len, temp16alen, temp16blen, temp16clen;
    PS_SIT temp32alen, temp32blen, temp48len, temp64len;
    REAL axtbb[8], axtcc[8], aytbb[8], aytcc[8];
    PS_SIT axtbblen, axtcclen, aytbblen, aytcclen;
    REAL bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
    PS_SIT bxtaalen, bxtcclen, bytaalen, bytcclen;
    REAL cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
    PS_SIT cxtaalen, cxtbblen, cytaalen, cytbblen;
    REAL axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
    PS_SIT axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
    REAL axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
    PS_SIT axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
    REAL axtbctt[8], aytbctt[8], bxtcatt[8];
    REAL bytcatt[8], cxtabtt[8], cytabtt[8];
    PS_SIT axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
    REAL abt[8], bct[8], cat[8];
    PS_SIT abtlen, bctlen, catlen;
    REAL abtt[4], bctt[4], catt[4];
    PS_SIT abttlen, bcttlen, cattlen;
    ...
    bc[3] = bc3;
    axbclen = ps_tri_scale_expansion_zeroelim(4, bc, adx, axbc);
    axxbclen = ps_tri_scale_expansion_zeroelim(axbclen, axbc, adx, axxbc);
    aybclen = ps_tri_scale_expansion_zeroelim(4, bc, ady, aybc);
    ayybclen = ps_tri_scale_expansion_zeroelim(aybclen, aybc, ady, ayybc);
    alen = ps_tri_fast_expansion_sum_zeroelim(axxbclen, axxbc, ayybclen, ayybc, adet);

    Two_Product(cdx, ady, cdxady1, cdxady0);
    Two_Product(adx, cdy, adxcdy1, adxcdy0);
    Two_Two_Diff(cdxady1, cdxady0, adxcdy1, adxcdy0, ca3, ca[2], ca[1], ca[0]);
    ca[3] = ca3;
    bxcalen = ps_tri_scale_expansion_zeroelim(4, ca, bdx, bxca);
    bxxcalen = ps_tri_scale_expansion_zeroelim(bxcalen, bxca, bdx, bxxca);
    bycalen = ps_tri_scale_expansion_zeroelim(4, ca, bdy, byca);
    byycalen = ps_tri_scale_expansion_zeroelim(bycalen, byca, bdy, byyca);
    blen = ps_tri_fast_expansion_sum_zeroelim(bxxcalen, bxxca, byycalen, byyca, bdet);

    Two_Product(adx, bdy, adxbdy1, adxbdy0);
    Two_Product(bdx, ady, bdxady1, bdxady0);
    Two_Two_Diff(adxbdy1, adxbdy0, bdxady1, bdxady0, ab3, ab[2], ab[1], ab[0]);
    ab[3] = ab3;
    cxablen = ps_tri_scale_expansion_zeroelim(4, ab, cdx, cxab);
    cxxablen = ps_tri_scale_expansion_zeroelim(cxablen, cxab, cdx, cxxab);
    cyablen = ps_tri_scale_expansion_zeroelim(4, ab, cdy, cyab);
    cyyablen = ps_tri_scale_expansion_zeroelim(cyablen, cyab, cdy, cyyab);
    clen = ps_tri_fast_expansion_sum_zeroelim(cxxablen, cxxab, cyyablen, cyyab, cdet);

    ablen = ps_tri_fast_expansion_sum_zeroelim(alen, adet, blen, bdet, abdet);
    finlength = ps_tri_fast_expansion_sum_zeroelim(ablen, abdet, clen, cdet, fin1);

    det = ps_tri_estimate(finlength, fin1);
    errbound = ps_tri_iccerrboundB * permanent;
    if ((det >= errbound) || (-det >= errbound)) {
        return det;
    }
    ...
    errbound = ps_tri_iccerrboundC * permanent + ps_tri_resulterrbound * Absolute(det);
    ...
    if (adxtail != 0.0) {
        axtbclen = ps_tri_scale_expansion_zeroelim(4, bc, adxtail, axtbc);
        temp16alen = ps_tri_scale_expansion_zeroelim(axtbclen, axtbc, 2.0 * adx,
                                                     temp16a);

        axtcclen = ps_tri_scale_expansion_zeroelim(4, cc, adxtail, axtcc);
        temp16blen = ps_tri_scale_expansion_zeroelim(axtcclen, axtcc, bdy, temp16b);

        axtbblen = ps_tri_scale_expansion_zeroelim(4, bb, adxtail, axtbb);
        temp16clen = ps_tri_scale_expansion_zeroelim(axtbblen, axtbb, -cdy, temp16c);

        temp32alen = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                        temp16blen, temp16b, temp32a);
        temp48len = ps_tri_fast_expansion_sum_zeroelim(temp16clen, temp16c,
                                                       temp32alen, temp32a, temp48);
        finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
                                                       temp48, finother);
        finswap = finnow; finnow = finother; finother = finswap;
    }
    if (adytail != 0.0) {
        aytbclen = ps_tri_scale_expansion_zeroelim(4, bc, adytail, aytbc);
        temp16alen = ps_tri_scale_expansion_zeroelim(aytbclen, aytbc, 2.0 * ady,
                                                     temp16a);

        aytbblen = ps_tri_scale_expansion_zeroelim(4, bb, adytail, aytbb);
        temp16blen = ps_tri_scale_expansion_zeroelim(aytbblen, aytbb, cdx, temp16b);

        aytcclen = ps_tri_scale_expansion_zeroelim(4, cc, adytail, aytcc);
        temp16clen = ps_tri_scale_expansion_zeroelim(aytcclen, aytcc, -bdx, temp16c);
    ...
    if (bdxtail != 0.0) {
        bxtcalen = ps_tri_scale_expansion_zeroelim(4, ca, bdxtail, bxtca);
        temp16alen = ps_tri_scale_expansion_zeroelim(bxtcalen, bxtca, 2.0 * bdx,
                                                     temp16a);

        bxtaalen = ps_tri_scale_expansion_zeroelim(4, aa, bdxtail, bxtaa);
        temp16blen = ps_tri_scale_expansion_zeroelim(bxtaalen, bxtaa, cdy, temp16b);

        bxtcclen = ps_tri_scale_expansion_zeroelim(4, cc, bdxtail, bxtcc);
        temp16clen = ps_tri_scale_expansion_zeroelim(bxtcclen, bxtcc, -ady, temp16c);
    ...
    if (bdytail != 0.0) {
        bytcalen = ps_tri_scale_expansion_zeroelim(4, ca, bdytail, bytca);
        temp16alen = ps_tri_scale_expansion_zeroelim(bytcalen, bytca, 2.0 * bdy,
                                                     temp16a);

        bytcclen = ps_tri_scale_expansion_zeroelim(4, cc, bdytail, bytcc);
        temp16blen = ps_tri_scale_expansion_zeroelim(bytcclen, bytcc, adx, temp16b);

        bytaalen = ps_tri_scale_expansion_zeroelim(4, aa, bdytail, bytaa);
        temp16clen = ps_tri_scale_expansion_zeroelim(bytaalen, bytaa, -cdx, temp16c);
    ...
    if (cdxtail != 0.0) {
        cxtablen = ps_tri_scale_expansion_zeroelim(4, ab, cdxtail, cxtab);
        temp16alen = ps_tri_scale_expansion_zeroelim(cxtablen, cxtab, 2.0 * cdx,
                                                     temp16a);

        cxtbblen = ps_tri_scale_expansion_zeroelim(4, bb, cdxtail, cxtbb);
        temp16blen = ps_tri_scale_expansion_zeroelim(cxtbblen, cxtbb, ady, temp16b);

        cxtaalen = ps_tri_scale_expansion_zeroelim(4, aa, cdxtail, cxtaa);
        temp16clen = ps_tri_scale_expansion_zeroelim(cxtaalen, cxtaa, -bdy, temp16c);
    ...
    if (cdytail != 0.0) {
        cytablen = ps_tri_scale_expansion_zeroelim(4, ab, cdytail, cytab);
        temp16alen = ps_tri_scale_expansion_zeroelim(cytablen, cytab, 2.0 * cdy,
                                                     temp16a);

        cytaalen = ps_tri_scale_expansion_zeroelim(4, aa, cdytail, cytaa);
        temp16blen = ps_tri_scale_expansion_zeroelim(cytaalen, cytaa, bdx, temp16b);

        cytbblen = ps_tri_scale_expansion_zeroelim(4, bb, cdytail, cytbb);
        temp16clen = ps_tri_scale_expansion_zeroelim(cytbblen, cytbb, -adx, temp16c);
    ...
    if ((adxtail != 0.0) || (adytail != 0.0)) {
        if ((bdxtail != 0.0) || (bdytail != 0.0)
            ...
            v[3] = v3;
            bctlen = ps_tri_fast_expansion_sum_zeroelim(4, u, 4, v, bct);
        ...
        if (adxtail != 0.0) {
            temp16alen = ps_tri_scale_expansion_zeroelim(axtbclen, axtbc, adxtail, temp16a);
            axtbctlen = ps_tri_scale_expansion_zeroelim(bctlen, bct, adxtail, axtbct);
            temp32alen = ps_tri_scale_expansion_zeroelim(axtbctlen, axtbct, 2.0 * adx,
                                                         temp32a);
            temp48len = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                           temp32alen, temp32a, temp48);
            finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
                                                           temp48, finother);
            finswap = finnow; finnow = finother; finother = finswap;
            if (bdytail != 0.0) {
                temp8len = ps_tri_scale_expansion_zeroelim(4, cc, adxtail, temp8);
                temp16alen = ps_tri_scale_expansion_zeroelim(temp8len, temp8, bdytail,
                                                             temp16a);
                finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                               temp16a, finother);
                finswap = finnow; finnow = finother; finother = finswap;
            }
            if (cdytail != 0.0) {
                temp8len = ps_tri_scale_expansion_zeroelim(4, bb, -adxtail, temp8);
                temp16alen = ps_tri_scale_expansion_zeroelim(temp8len, temp8, cdytail,
                                                             temp16a);
                finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                               temp16a, finother);
                finswap = finnow; finnow = finother; finother = finswap;
            ...
            temp32alen = ps_tri_scale_expansion_zeroelim(axtbctlen, axtbct, adxtail,
                                                         temp32a);
            axtbcttlen = ps_tri_scale_expansion_zeroelim(bcttlen, bctt, adxtail, axtbctt);
            temp16alen = ps_tri_scale_expansion_zeroelim(axtbcttlen, axtbctt, 2.0 * adx,
                                                         temp16a);
            temp16blen = ps_tri_scale_expansion_zeroelim(axtbcttlen, axtbctt, adxtail,
                                                         temp16b);
            temp32blen = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                            temp16blen, temp16b, temp32b);
            temp64len = ps_tri_fast_expansion_sum_zeroelim(temp32alen, temp32a,
                                                           temp32blen, temp32b, temp64);
            finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp64len,
                                                           temp64, finother);
            finswap = finnow; finnow = finother; finother = finswap;
        }
        if (adytail != 0.0) {
            temp16alen = ps_tri_scale_expansion_zeroelim(aytbclen, aytbc, adytail, temp16a);
            aytbctlen = ps_tri_scale_expansion_zeroelim(bctlen, bct, adytail, aytbct);
            temp32alen = ps_tri_scale_expansion_zeroelim(aytbctlen, aytbct, 2.0 * ady,
                                                         temp32a);
            temp48len = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                           temp32alen, temp32a, temp48);
            finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
            ...
            temp32alen = ps_tri_scale_expansion_zeroelim(aytbctlen, aytbct, adytail,
                                                         temp32a);
            aytbcttlen = ps_tri_scale_expansion_zeroelim(bcttlen, bctt, adytail, aytbctt);
            temp16alen = ps_tri_scale_expansion_zeroelim(aytbcttlen, aytbctt, 2.0 * ady,
                                                         temp16a);
            temp16blen = ps_tri_scale_expansion_zeroelim(aytbcttlen, aytbctt, adytail,
                                                         temp16b);
            temp32blen = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                            temp16blen, temp16b, temp32b);
            temp64len = ps_tri_fast_expansion_sum_zeroelim(temp32alen, temp32a,
    ...
    if ((bdxtail != 0.0) || (bdytail != 0.0)) {
        if ((cdxtail != 0.0) || (cdytail != 0.0)
            ...
            Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
            v[3] = v3;
            catlen = ps_tri_fast_expansion_sum_zeroelim(4, u, 4, v, cat);
        ...
        if (bdxtail != 0.0) {
            temp16alen = ps_tri_scale_expansion_zeroelim(bxtcalen, bxtca, bdxtail, temp16a);
            bxtcatlen = ps_tri_scale_expansion_zeroelim(catlen, cat, bdxtail, bxtcat);
            temp32alen = ps_tri_scale_expansion_zeroelim(bxtcatlen, bxtcat, 2.0 * bdx,
                                                         temp32a);
            temp48len = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                           temp32alen, temp32a, temp48);
            finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
            ...
            finswap = finnow; finnow = finother; finother = finswap;
            if (cdytail != 0.0) {
                temp8len = ps_tri_scale_expansion_zeroelim(4, aa, bdxtail, temp8);
                temp16alen = ps_tri_scale_expansion_zeroelim(temp8len, temp8, cdytail,
                                                             temp16a);
                finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                               temp16a, finother);
            ...
            if (adytail != 0.0) {
                temp8len = ps_tri_scale_expansion_zeroelim(4, cc, -bdxtail, temp8);
                temp16alen = ps_tri_scale_expansion_zeroelim(temp8len, temp8, adytail,
                                                             temp16a);
                finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                               temp16a, finother);
                finswap = finnow; finnow = finother; finother = finswap;
            ...
            temp32alen = ps_tri_scale_expansion_zeroelim(bxtcatlen, bxtcat, bdxtail,
                                                         temp32a);
            bxtcattlen = ps_tri_scale_expansion_zeroelim(cattlen, catt, bdxtail, bxtcatt);
            temp16alen = ps_tri_scale_expansion_zeroelim(bxtcattlen, bxtcatt, 2.0 * bdx,
                                                         temp16a);
            temp16blen = ps_tri_scale_expansion_zeroelim(bxtcattlen, bxtcatt, bdxtail,
                                                         temp16b);
            temp32blen = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                            temp16blen, temp16b, temp32b);
            temp64len = ps_tri_fast_expansion_sum_zeroelim(temp32alen, temp32a,
        ...
        if (bdytail != 0.0) {
            temp16alen = ps_tri_scale_expansion_zeroelim(bytcalen, bytca, bdytail, temp16a);
            bytcatlen = ps_tri_scale_expansion_zeroelim(catlen, cat, bdytail, bytcat);
            temp32alen = ps_tri_scale_expansion_zeroelim(bytcatlen, bytcat, 2.0 * bdy,
                                                         temp32a);
            temp48len = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                           temp32alen, temp32a, temp48);
            finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
            ...
            temp32alen = ps_tri_scale_expansion_zeroelim(bytcatlen, bytcat, bdytail,
                                                         temp32a);
            bytcattlen = ps_tri_scale_expansion_zeroelim(cattlen, catt, bdytail, bytcatt);
            temp16alen = ps_tri_scale_expansion_zeroelim(bytcattlen, bytcatt, 2.0 * bdy,
                                                         temp16a);
            temp16blen = ps_tri_scale_expansion_zeroelim(bytcattlen, bytcatt, bdytail,
                                                         temp16b);
            temp32blen = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                            temp16blen, temp16b, temp32b);
            temp64len = ps_tri_fast_expansion_sum_zeroelim(temp32alen, temp32a,
    ...
    if ((cdxtail != 0.0) || (cdytail != 0.0)) {
        if ((adxtail != 0.0) || (adytail != 0.0)
            ...
            Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
            v[3] = v3;
            abtlen = ps_tri_fast_expansion_sum_zeroelim(4, u, 4, v, abt);
        ...
        if (cdxtail != 0.0) {
            temp16alen = ps_tri_scale_expansion_zeroelim(cxtablen, cxtab, cdxtail, temp16a);
            cxtabtlen = ps_tri_scale_expansion_zeroelim(abtlen, abt, cdxtail, cxtabt);
            temp32alen = ps_tri_scale_expansion_zeroelim(cxtabtlen, cxtabt, 2.0 * cdx,
                                                         temp32a);
            temp48len = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                           temp32alen, temp32a, temp48);
            finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
            ...
            finswap = finnow; finnow = finother; finother = finswap;
            if (adytail != 0.0) {
                temp8len = ps_tri_scale_expansion_zeroelim(4, bb, cdxtail, temp8);
                temp16alen = ps_tri_scale_expansion_zeroelim(temp8len, temp8, adytail,
                                                             temp16a);
                finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                               temp16a, finother);
            ...
            if (bdytail != 0.0) {
                temp8len = ps_tri_scale_expansion_zeroelim(4, aa, -cdxtail, temp8);
                temp16alen = ps_tri_scale_expansion_zeroelim(temp8len, temp8, bdytail,
                                                             temp16a);
                finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp16alen,
                                                               temp16a, finother);
            ...
            temp32alen = ps_tri_scale_expansion_zeroelim(cxtabtlen, cxtabt, cdxtail,
                                                         temp32a);
            cxtabttlen = ps_tri_scale_expansion_zeroelim(abttlen, abtt, cdxtail, cxtabtt);
            temp16alen = ps_tri_scale_expansion_zeroelim(cxtabttlen, cxtabtt, 2.0 * cdx,
                                                         temp16a);
            temp16blen = ps_tri_scale_expansion_zeroelim(cxtabttlen, cxtabtt, cdxtail,
                                                         temp16b);
            temp32blen = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                            temp16blen, temp16b, temp32b);
            temp64len = ps_tri_fast_expansion_sum_zeroelim(temp32alen, temp32a,
        ...
        if (cdytail != 0.0) {
            temp16alen = ps_tri_scale_expansion_zeroelim(cytablen, cytab, cdytail, temp16a);
            cytabtlen = ps_tri_scale_expansion_zeroelim(abtlen, abt, cdytail, cytabt);
            temp32alen = ps_tri_scale_expansion_zeroelim(cytabtlen, cytabt, 2.0 * cdy,
                                                         temp32a);
            temp48len = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                           temp32alen, temp32a, temp48);
            finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, temp48len,
            ...
            temp32alen = ps_tri_scale_expansion_zeroelim(cytabtlen, cytabt, cdytail,
                                                         temp32a);
            cytabttlen = ps_tri_scale_expansion_zeroelim(abttlen, abtt, cdytail, cytabtt);
            temp16alen = ps_tri_scale_expansion_zeroelim(cytabttlen, cytabtt, 2.0 * cdy,
                                                         temp16a);
            temp16blen = ps_tri_scale_expansion_zeroelim(cytabttlen, cytabtt, cdytail,
                                                         temp16b);
            temp32blen = ps_tri_fast_expansion_sum_zeroelim(temp16alen, temp16a,
                                                            temp16blen, temp16b, temp32b);
            temp64len = ps_tri_fast_expansion_sum_zeroelim(temp32alen, temp32a,
...
#ifdef ANSI_DECLARATORS
REAL ps_tri_incircle(struct mesh *m, struct behavior *b,
                     vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_incircle(m, b, pa, pb, pc, pd)
    struct mesh *m;
    struct behavior *b;
    vertex pa;
    vertex pb;
...

{
    ...
    errbound = ps_tri_iccerrboundA * permanent;
    if ((det > errbound) || (-det > errbound)) {
        return det;
    }

    return ps_tri_incircleadapt(pa, pb, pc, pd, permanent);
}
...
#ifdef ANSI_DECLARATORS
REAL ps_tri_orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
                          REAL aheight, REAL bheight, REAL cheight, REAL dheight,
                          REAL permanent)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_orient3dadapt(pa, pb, pc, pd,
...

{
    ...
    PS_SIT at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
    ...
    Two_Two_Diff(bdxcdy1, bdxcdy0, cdxbdy1, cdxbdy0, bc3, bc[2], bc[1], bc[0]);
    bc[3] = bc3;
    alen = ps_tri_scale_expansion_zeroelim(4, bc, adheight, adet);

    Two_Product(cdx, ady, cdxady1, cdxady0);
    Two_Product(adx, cdy, adxcdy1, adxcdy0);
    Two_Two_Diff(cdxady1, cdxady0, adxcdy1, adxcdy0, ca3, ca[2], ca[1], ca[0]);
    ca[3] = ca3;
    blen = ps_tri_scale_expansion_zeroelim(4, ca, bdheight, bdet);

    Two_Product(adx, bdy, adxbdy1, adxbdy0);
    Two_Product(bdx, ady, bdxady1, bdxady0);
    Two_Two_Diff(adxbdy1, adxbdy0, bdxady1, bdxady0, ab3, ab[2], ab[1], ab[0]);
    ab[3] = ab3;
    clen = ps_tri_scale_expansion_zeroelim(4, ab, cdheight, cdet);

    ablen = ps_tri_fast_expansion_sum_zeroelim(alen, adet, blen, bdet, abdet);
    finlength = ps_tri_fast_expansion_sum_zeroelim(ablen, abdet, clen, cdet, fin1);

    det = ps_tri_estimate(finlength, fin1);
    errbound = ps_tri_o3derrboundB * permanent;
    if ((det >= errbound) || (-det >= errbound)) {
        return det;
    }
    ...
    errbound = ps_tri_o3derrboundC * permanent + ps_tri_resulterrbound * Absolute(det);
    ...
    bctlen = ps_tri_fast_expansion_sum_zeroelim(bt_clen, bt_c, ct_blen, ct_b, bct);
    wlength = ps_tri_scale_expansion_zeroelim(bctlen, bct, adheight, w);
    finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                                   finother);
    finswap = finnow; finnow = finother; finother = finswap;

    catlen = ps_tri_fast_expansion_sum_zeroelim(ct_alen, ct_a, at_clen, at_c, cat);
    wlength = ps_tri_scale_expansion_zeroelim(catlen, cat, bdheight, w);
    finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                                   finother);
    finswap = finnow; finnow = finother; finother = finswap;

    abtlen = ps_tri_fast_expansion_sum_zeroelim(at_blen, at_b, bt_alen, bt_a, abt);
    wlength = ps_tri_scale_expansion_zeroelim(abtlen, abt, cdheight, w);
    finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                                   finother);
    finswap = finnow; finnow = finother; finother = finswap;

    if (adheighttail != 0.0) {
        vlength = ps_tri_scale_expansion_zeroelim(4, bc, adheighttail, v);
        finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, vlength, v,
                                                       finother);
        finswap = finnow; finnow = finother; finother = finswap;
    }
    if (bdheighttail != 0.0) {
        vlength = ps_tri_scale_expansion_zeroelim(4, ca, bdheighttail, v);
        finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, vlength, v,
                                                       finother);
        finswap = finnow; finnow = finother; finother = finswap;
    }
    if (cdheighttail != 0.0) {
        vlength = ps_tri_scale_expansion_zeroelim(4, ab, cdheighttail, v);
        finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, vlength, v,
                                                       finother);
        finswap = finnow; finnow = finother; finother = finswap;
    }
    ...
    if (adxtail != 0.0) {
        if (bdytail != 0.0) {
            ...
            u[3] = u3;
            finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                                                           finother);
            finswap = finnow; finnow = finother; finother = finswap;
            if (cdheighttail != 0.0) {
    ...
    if (adheighttail != 0.0) {
        wlength = ps_tri_scale_expansion_zeroelim(bctlen, bct, adheighttail, w);
        finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                                       finother);
        finswap = finnow; finnow = finother; finother = finswap;
    }
    if (bdheighttail != 0.0) {
        wlength = ps_tri_scale_expansion_zeroelim(catlen, cat, bdheighttail, w);
        finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                                       finother);
        finswap = finnow; finnow = finother; finother = finswap;
    }
    if (cdheighttail != 0.0) {
        wlength = ps_tri_scale_expansion_zeroelim(abtlen, abt, cdheighttail, w);
        finlength = ps_tri_fast_expansion_sum_zeroelim(finlength, finnow, wlength, w,
                                                       finother);
        finswap = finnow; finnow = finother; finother = finswap;
    }
...
#ifdef ANSI_DECLARATORS
REAL ps_tri_orient3d(struct mesh *m, struct behavior *b,
                     vertex pa, vertex pb, vertex pc, vertex pd,
                     REAL aheight, REAL bheight, REAL cheight, REAL dheight)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
    struct mesh *m;
    struct behavior *b;
    vertex pa;
    vertex pb;
...

{
    ...
    errbound = ps_tri_o3derrboundA * permanent;
    if ((det > errbound) || (-det > errbound)) {
        return det;
    }

    return ps_tri_orient3dadapt(pa, pb, pc, pd, aheight, bheight, cheight, dheight,
                                permanent);
}
...
#ifdef ANSI_DECLARATORS
REAL ps_tri_nonregular(struct mesh *m, struct behavior *b,
                       vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_nonregular(m, b, pa, pb, pc, pd)
    struct mesh *m;
    struct behavior *b;
    vertex pa;
    vertex pb;
...
{
    if (b->weighted == 0) {
        return ps_tri_incircle(m, b, pa, pb, pc, pd);
    } else if (b->weighted == 1) {
        return ps_tri_orient3d(m, b, pa, pb, pc, pd,
                               pa[0] * pa[0] + pa[1] * pa[1] - pa[2],
                               pb[0] * pb[0] + pb[1] * pb[1] - pb[2],
                               pc[0] * pc[0] + pc[1] * pc[1] - pc[2],
                               pd[0] * pd[0] + pd[1] * pd[1] - pd[2]);
    } else {
        return ps_tri_orient3d(m, b, pa, pb, pc, pd, pa[2], pb[2], pc[2], pd[2]);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_findcircumcenter(struct mesh *m, struct behavior *b,
                             vertex torg, vertex tdest, vertex tapex,
                             vertex circumcenter, REAL *xi, REAL *eta, PS_SIT offcenter)
#else /* not ANSI_DECLARATORS */
    void ps_tri_findcircumcenter(m, b, torg, tdest, tapex, circumcenter, xi, eta,
                                 offcenter)
    struct mesh *m;
    struct behavior *b;
    vertex torg;
...

{
    ...
    if (b->noexact) {
        ...
        /*   division by zero.                                          */
        denominator = 0.5 / ps_tri_counterclockwise(m, b, tdest, tapex, torg);
...
#ifdef ANSI_DECLARATORS
void ps_tri_triangleinit(struct mesh *m)
#else /* not ANSI_DECLARATORS */
    void ps_tri_triangleinit(m)
    struct mesh *m;
#endif /* not ANSI_DECLARATORS */

{
    ps_tri_poolzero(&m->vertices);
    ps_tri_poolzero(&m->triangles);
    ps_tri_poolzero(&m->subsegs);
    ps_tri_poolzero(&m->viri);
    ps_tri_poolzero(&m->badsubsegs);
    ps_tri_poolzero(&m->badtriangles);
    ps_tri_poolzero(&m->flipstackers);
    ps_tri_poolzero(&m->splaynodes);
    ...
    ps_tri_exactinit();                     /* Initialize exact arithmetic constants. */
}
...
#ifdef ANSI_DECLARATORS
unsigned long long ps_tri_randomnation(PS_unsignedSIT choices)
#else /* not ANSI_DECLARATORS */
    unsigned long long ps_tri_randomnation(choices)
    PS_unsignedSIT choices;
#endif /* not ANSI_DECLARATORS */

{
    ps_tri_randomseed = (ps_tri_randomseed * 1366l + 150889l) % 714025l;
    return ps_tri_randomseed / (714025l / choices + 1);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_checkmesh(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_checkmesh(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_traversalinit(&m->triangles);
    triangleloop.tri = ps_tri_triangletraverse(m);
    while (triangleloop.tri != (triangle *) NULL) {
        ...
        for (triangleloop.orient = 0; triangleloop.orient < 3;
            ...
            if (triangleloop.orient == 0) {       /* Only test for inversion once. */
                ...
                if (ps_tri_counterclockwise(m, b, triorg, tridest, triapex) <= 0.0) {
                    printf("  !! !! Inverted ");
                    ps_tri_printtriangle(m, b, &triangleloop);
                    horrors++;
                }
            }
            ...
            if (oppotri.tri != m->dummytri) {
                ...
                if ((triangleloop.tri != oppooppotri.tri)
                    ...
                    ps_tri_printtriangle(m, b, &oppotri);
                    horrors++;
                }
    ...
    if (horrors == 0) {
        ...
        printf("  !! !! !! !! %lld abominations witnessed.\n", (long long)horrors);
    }
...
#ifdef ANSI_DECLARATORS
void ps_tri_checkdelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_checkdelaunay(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    while (triangleloop.tri != (triangle *) NULL) {
        ...
        for (triangleloop.orient = 0; triangleloop.orient < 3;
            ...
            if (shouldbedelaunay) {
                if (ps_tri_nonregular(m, b, triorg, tridest, triapex, oppoapex) > 0.0) {
                    if (!b->weighted) {
    ...
    if (horrors == 0) {
        ...
        printf("  !! !! !! !! %lld obscenities viewed with horror.\n", (long long)horrors);
    }
    /* Restore the status of exact arithmetic. */
    b->noexact = saveexact;
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_enqueuebadtriang(struct mesh *m, struct behavior *b,
                             struct badtriang *badtri)
#else /* not ANSI_DECLARATORS */
    void ps_tri_enqueuebadtriang(m, b, badtri)
    struct mesh *m;
    struct behavior *b;
    struct badtriang *badtri;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
                          REAL minedge, vertex enqapex, vertex enqorg, vertex enqdest)
#else /* not ANSI_DECLARATORS */
    void ps_tri_enqueuebadtri(m, b, enqtri, minedge, enqapex, enqorg, enqdest)
    struct mesh *m;
    struct behavior *b;
    struct otri *enqtri;
    REAL minedge;
...
#ifdef ANSI_DECLARATORS
struct badtriang *ps_tri_dequeuebadtriang(struct mesh *m)
#else /* not ANSI_DECLARATORS */
    struct badtriang *ps_tri_dequeuebadtriang(m)
    struct mesh *m;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
PS_SIT ps_tri_checkseg4encroach(struct mesh *m, struct behavior *b,
                                struct osub *testsubseg)
#else /* not ANSI_DECLARATORS */
    PS_SIT ps_tri_checkseg4encroach(m, b, testsubseg)
    struct mesh *m;
    struct behavior *b;
    struct osub *testsubseg;
#endif /* not ANSI_DECLARATORS */

{
    ...
    if (encroached && (!b->nobisect || ((b->nobisect == 1) && (sides == 2)))) {
        ...
        encroachedseg = (struct badsubseg *) ps_tri_poolalloc(&m->badsubsegs);
        if (encroached == 1) {
...
#ifdef ANSI_DECLARATORS
void ps_tri_testtriangle(struct mesh *m, struct behavior *b, struct otri *testtri)
#else /* not ANSI_DECLARATORS */
    void ps_tri_testtriangle(m, b, testtri)
    struct mesh *m;
    struct behavior *b;
    struct otri *testtri;
#endif /* not ANSI_DECLARATORS */

{
    ...
    if (b->vararea || b->fixedarea || b->usertest) {
        ...
        if (b->fixedarea && (area > b->maxarea)) {
            ...
            ps_tri_enqueuebadtri(m, b, testtri, minedge, tapex, torg, tdest);
            return;
        }
        ...
        if (b->usertest) {
            ...
            if (ps_tri_triunsuitable(torg, tdest, tapex, area)) {
                ps_tri_enqueuebadtri(m, b, testtri, minedge, tapex, torg, tdest);
                return;
            }
        }
...
#ifdef ANSI_DECLARATORS
void ps_tri_makevertexmap(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_makevertexmap(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
enum locateresult ps_tri_preciselocate(struct mesh *m, struct behavior *b,
                                       vertex searchpoint, struct otri *searchtri,
                                       PS_SIT stopatsubsegment)
#else /* not ANSI_DECLARATORS */
    enum locateresult ps_tri_preciselocate(m, b, searchpoint, searchtri, stopatsubsegment)
    struct mesh *m;
    struct behavior *b;
    vertex searchpoint;
...

{
    ...
    while (1) {
        ...
        destorient = ps_tri_counterclockwise(m, b, forg, fapex, searchpoint);
        /* Does the point lie on the other side of the line defined by the */
        /*   triangle edge opposite the triangle's origin?                 */
        orgorient = ps_tri_counterclockwise(m, b, fapex, fdest, searchpoint);
        if (destorient > 0.0) {
            if (orgorient > 0.0) {
...
#ifdef ANSI_DECLARATORS
enum locateresult ps_tri_locate(struct mesh *m, struct behavior *b,
                                vertex searchpoint, struct otri *searchtri)
#else /* not ANSI_DECLARATORS */
    enum locateresult ps_tri_locate(m, b, searchpoint, searchtri)
    struct mesh *m;
    struct behavior *b;
    vertex searchpoint;
    struct otri *searchtri;
...

{
    ...
    while (totalsamplesleft > 0) {
        ...
        do {
            sampletri.tri = (triangle *) (firsttri +
                                          (ps_tri_randomnation((PS_unsignedSIT) population) *
    ...
    ahead = ps_tri_counterclockwise(m, b, torg, tdest, searchpoint);
    if (ahead < 0.0) {
    ...
    return ps_tri_preciselocate(m, b, searchpoint, searchtri, 0);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_insertsubseg(struct mesh *m, struct behavior *b, struct otri *tri,
                         PS_SIT subsegmark)
#else /* not ANSI_DECLARATORS */
    void ps_tri_insertsubseg(m, b, tri, subsegmark)
    struct mesh *m;
    struct behavior *b;
    struct otri *tri;             /* Edge at which to insert the new subsegment. */
    PS_SIT subsegmark;                            /* Marker for the new subsegment. */
#endif /* not ANSI_DECLARATORS */

{
    ...
    if (newsubseg.ss == m->dummysub) {
        ...
        if (b->verbose > 2) {
            ...
            ps_tri_printsubseg(m, b, &newsubseg);
        }
    } else {
        if (mark(newsubseg) == 0) {
            setmark(newsubseg, subsegmark);
...
#ifdef ANSI_DECLARATORS
void ps_tri_flip(struct mesh *m, struct behavior *b, struct otri *flipedge)
#else /* not ANSI_DECLARATORS */
    void ps_tri_flip(m, b, flipedge)
    struct mesh *m;
    struct behavior *b;
...

{
    ...
    if (b->verbose > 2) {
        ...
        ps_tri_printtriangle(m, b, &top);
        printf("  and right ");
        ps_tri_printtriangle(m, b, flipedge);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_unflip(struct mesh *m, struct behavior *b, struct otri *flipedge)
#else /* not ANSI_DECLARATORS */
    void ps_tri_unflip(m, b, flipedge)
    struct mesh *m;
    struct behavior *b;
    struct otri *flipedge;                    /* Handle for the triangle abc. */
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
enum insertvertexresult ps_tri_insertvertex(struct mesh *m, struct behavior *b,
                                            vertex newvertex, struct otri *searchtri,
                                            struct osub *splitseg,
                                            PS_SIT segmentflaws, PS_SIT triflaws)
#else /* not ANSI_DECLARATORS */
    enum insertvertexresult ps_tri_insertvertex(m, b, newvertex, searchtri, splitseg,
...

{
    ...
    if (splitseg == (struct osub *) NULL) {
        ...
        if (searchtri->tri == m->dummytri) {
            ...
            intersect = ps_tri_locate(m, b, newvertex, &horiz);
        } else {
            /* Start searching from the triangle provided by the caller. */
            otricopy(*searchtri, horiz);
            intersect = ps_tri_preciselocate(m, b, newvertex, &horiz, 1);
        }
    } else {
    ...
    if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
        ...
        if (m->checksegments && (splitseg == (struct osub *) NULL)) {
            ...
            if (brokensubseg.ss != m->dummysub) {
                ...
                if (segmentflaws) {
                    ...
                    if (enq) {
                        /* Add the subsegment to the list of encroached subsegments. */
                        encroached = (struct badsubseg *) ps_tri_poolalloc(&m->badsubsegs);
        ...
        if (mirrorflag) {
            lnextself(topright);
            sym(topright, toprcasing);
            ps_tri_maketriangle(m, b, &newtopright);
        } else {
            /* Splitting a boundary edge increases the number of boundary edges. */
            m->hullsize++;
        }
        ps_tri_maketriangle(m, b, &newbotright);
        ...
        if (splitseg != (struct osub *) NULL) {
            ...
            ps_tri_insertsubseg(m, b, &newbotright, mark(*splitseg));
        ...
        if (m->checkquality) {
            ps_tri_poolrestart(&m->flipstackers);
            m->lastflip = (struct flipstacker *) ps_tri_poolalloc(&m->flipstackers);
            m->lastflip->flippedtri = encode(horiz);
            m->lastflip->prevflip = (struct flipstacker *) &ps_tri_insertvertex;
        }

#ifdef SELF_CHECK
        if (ps_tri_counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
            printf("Internal error in insertvertex():\n");
            printf(
                "  Clockwise triangle prior to edge vertex insertion (bottom).\n");
        }
        if (mirrorflag) {
            if (ps_tri_counterclockwise(m, b, leftvertex, rightvertex, topvertex) < 0.0) {
                printf("Internal error in insertvertex():\n");
                printf("  Clockwise triangle prior to edge vertex insertion (top).\n");
            }
            if (ps_tri_counterclockwise(m, b, rightvertex, topvertex, newvertex) < 0.0) {
                printf("Internal error in insertvertex():\n");
                printf(
                    "  Clockwise triangle after edge vertex insertion (top right).\n");
            }
            if (ps_tri_counterclockwise(m, b, topvertex, leftvertex, newvertex) < 0.0) {
                printf("Internal error in insertvertex():\n");
                printf(
                    "  Clockwise triangle after edge vertex insertion (top left).\n");
            }
        }
        if (ps_tri_counterclockwise(m, b, leftvertex, botvertex, newvertex) < 0.0) {
            printf("Internal error in insertvertex():\n");
            printf(
                "  Clockwise triangle after edge vertex insertion (bottom left).\n");
        }
        if (ps_tri_counterclockwise(m, b, botvertex, rightvertex, newvertex) < 0.0) {
            printf("Internal error in insertvertex():\n");
            printf(
        ...
        if (b->verbose > 2) {
            ...
            ps_tri_printtriangle(m, b, &botright);
            if (mirrorflag) {
                printf("  Updating top left ");
                ps_tri_printtriangle(m, b, &topright);
                printf("  Creating top right ");
                ps_tri_printtriangle(m, b, &newtopright);
            }
            printf("  Creating bottom right ");
            ps_tri_printtriangle(m, b, &newbotright);
        }
        ...
        sym(botleft, botlcasing);
        sym(botright, botrcasing);
        ps_tri_maketriangle(m, b, &newbotleft);
        ps_tri_maketriangle(m, b, &newbotright);
        ...
        if (ps_tri_counterclockwise(m, b, rightvertex, leftvertex, newvertex) < 0.0) {
            printf("Internal error in insertvertex():\n");
        ...
        if (b->verbose > 2) {
            printf("  Updating top ");
            ps_tri_printtriangle(m, b, &horiz);
            printf("  Creating left ");
            ps_tri_printtriangle(m, b, &newbotleft);
        ...
        if (m->checksegments) {
            ...
            if (checksubseg.ss != m->dummysub) {
                ...
                if (segmentflaws) {
                    ...
                    if (ps_tri_checkseg4encroach(m, b, &checksubseg)) {
        ...
        if (doflip) {
            ...
            if (top.tri == m->dummytri) {
                ...
                if ((leftvertex == m->infvertex1) || (leftvertex == m->infvertex2) ||
                    ...
                    /*   be applied.                                                */
                    doflip = ps_tri_counterclockwise(m, b, newvertex, rightvertex, farvertex)
                        > 0.0;
                    ...
                    /*   infinite as well, but trust me, this same condition should */
                    /*   be applied.                                                */
                    doflip = ps_tri_counterclockwise(m, b, farvertex, leftvertex, newvertex)
                        > 0.0;
                    ...
                    doflip = ps_tri_incircle(m, b, leftvertex, newvertex, rightvertex,
                                             farvertex) > 0.0;
                }
                if (doflip) {
                    ...
                    if (m->checkquality) {
                        newflip = (struct flipstacker *) ps_tri_poolalloc(&m->flipstackers);
...
#ifdef SELF_CHECK
                    if (newvertex != (vertex) NULL) {
                        if (ps_tri_counterclockwise(m, b, leftvertex, newvertex, rightvertex) <
                            0.0) {
                            printf("Internal error in insertvertex():\n");
                        ...
                        /*   removes.                                                    */
/*
  if (ps_tri_counterclockwise(m, b, rightvertex, farvertex, leftvertex) <
  0.0) {
  printf("Internal error in insertvertex():\n");
  printf("  Clockwise triangle prior to edge flip (top).\n");
  }
*/
                        if (ps_tri_counterclockwise(m, b, farvertex, leftvertex, newvertex) <
                            0.0) {
                            printf("Internal error in insertvertex():\n");
                            printf("  Clockwise triangle after edge flip (left).\n");
                        }
                        if (ps_tri_counterclockwise(m, b, newvertex, rightvertex, farvertex) <
                            0.0) {
                            printf("Internal error in insertvertex():\n");
                    ...
                    if (b->verbose > 2) {
                        printf("  Edge flip results in left ");
                        lnextself(topleft);
                        ps_tri_printtriangle(m, b, &topleft);
                        printf("  and right ");
                        ps_tri_printtriangle(m, b, &horiz);
                    }
...
#ifdef ANSI_DECLARATORS
void ps_tri_triangulatepolygon(struct mesh *m, struct behavior *b,
                               struct otri *firstedge, struct otri *lastedge,
                               PS_SIT edgecount, PS_SIT doflip, PS_SIT triflaws)
#else /* not ANSI_DECLARATORS */
    void ps_tri_triangulatepolygon(m, b, firstedge, lastedge, edgecount, doflip, triflaws)
    struct mesh *m;
    struct behavior *b;
...

{
    ...
    for (i = 2; i <= edgecount - 2; i++) {
        ...
        if (ps_tri_incircle(m, b, leftbasevertex, rightbasevertex, bestvertex,
                            testvertex) > 0.0) {
    ...
    if (bestnumber > 1) {
        ...
        ps_tri_triangulatepolygon(m, b, firstedge, &tempedge, bestnumber + 1, 1,
                                  triflaws);
    }
    if (bestnumber < edgecount - 2) {
        /* Recursively triangulate the smaller polygon on the left. */
        sym(besttri, tempedge);
        ps_tri_triangulatepolygon(m, b, &besttri, lastedge, edgecount - bestnumber, 1,
                                  triflaws);
    ...
    if (doflip) {
        ...
        ps_tri_flip(m, b, &besttri);
#ifndef CDT_ONLY
        if (triflaws) {
            /* Check the quality of the newly committed triangle. */
            sym(besttri, testtri);
            ps_tri_testtriangle(m, b, &testtri);
        }
#endif /* not CDT_ONLY */
    }
...
#ifdef ANSI_DECLARATORS
void ps_tri_deletevertex(struct mesh *m, struct behavior *b, struct otri *deltri)
#else /* not ANSI_DECLARATORS */
    void ps_tri_deletevertex(m, b, deltri)
    struct mesh *m;
    struct behavior *b;
    struct otri *deltri;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_vertexdealloc(m, delvertex);
    ...
    if (edgecount > 3) {
        ...
        ps_tri_triangulatepolygon(m, b, &firstedge, &lastedge, edgecount, 0,
                                  !b->nobisect);
    }
    ...
    if (!b->nobisect) {
        ps_tri_testtriangle(m, b, deltri);
    }

    /* Delete the two spliced-out triangles. */
    ps_tri_triangledealloc(m, lefttri.tri);
    ps_tri_triangledealloc(m, righttri.tri);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_undovertex(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_undovertex(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    while (m->lastflip != (struct flipstacker *) NULL) {
        ...
        if (m->lastflip->prevflip == (struct flipstacker *) NULL) {
            ...
            /* Delete the two spliced-out triangles. */
            ps_tri_triangledealloc(m, botleft.tri);
            ps_tri_triangledealloc(m, botright.tri);
        } else if (m->lastflip->prevflip == (struct flipstacker *) &ps_tri_insertvertex) {
            ...
            if (gluetri.tri != m->dummytri) {
                ...
                /* Delete the spliced-out triangle. */
                ps_tri_triangledealloc(m, topright.tri);
            }
...
#ifdef ANSI_DECLARATORS
void ps_tri_vertexsort(vertex *sortarray, PS_SIT arraysize)
#else /* not ANSI_DECLARATORS */
    void ps_tri_vertexsort(sortarray, arraysize)
    vertex *sortarray;
    PS_SIT arraysize;
#endif /* not ANSI_DECLARATORS */

{
    ...
    pivot = (PS_SIT) ps_tri_randomnation((PS_unsignedSIT) arraysize);
    ...
    if (left > 1) {
        ...
        ps_tri_vertexsort(sortarray, left);
    }
    if (right < arraysize - 2) {
        /* Recursively sort the right subset. */
        ps_tri_vertexsort(&sortarray[right + 1], arraysize - right - 1);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_vertexmedian(vertex *sortarray, PS_SIT arraysize, PS_SIT median, PS_SIT axis)
#else /* not ANSI_DECLARATORS */
    void ps_tri_vertexmedian(sortarray, arraysize, median, axis)
    vertex *sortarray;
    PS_SIT arraysize;
    PS_SIT median;
    PS_SIT axis;
...

{
    ...
    if (left > median) {
        ...
        ps_tri_vertexmedian(sortarray, left, median, axis);
    }
    if (right < median - 1) {
        /* Recursively shuffle the right subset. */
        ps_tri_vertexmedian(&sortarray[right + 1], arraysize - right - 1,
                            median - right - 1, axis);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_alternateaxes(vertex *sortarray, PS_SIT arraysize, PS_SIT axis)
#else /* not ANSI_DECLARATORS */
    void ps_tri_alternateaxes(sortarray, arraysize, axis)
    vertex *sortarray;
    PS_SIT arraysize;
    PS_SIT axis;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_vertexmedian(sortarray, arraysize, divider, axis);
    /* Recursively partition the subsets with a cross cut. */
    if (arraysize - divider >= 2) {
        if (divider >= 2) {
            ps_tri_alternateaxes(sortarray, divider, 1 - axis);
        }
        ps_tri_alternateaxes(&sortarray[divider], arraysize - divider, 1 - axis);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_mergehulls(struct mesh *m, struct behavior *b, struct otri *farleft,
                       struct otri *innerleft, struct otri *innerright,
                       struct otri *farright, PS_SIT axis)
#else /* not ANSI_DECLARATORS */
    void ps_tri_mergehulls(m, b, farleft, innerleft, innerright, farright, axis)
    struct mesh *m;
    struct behavior *b;
    struct otri *farleft;
...

{
    ...
    PS_SIT changemade;
    PS_SIT badedge;
    PS_SIT leftfinished, rightfinished;
    triangle ptr;                         /* Temporary variable used by sym(). */
    ...
    do {
        ...
        if (ps_tri_counterclockwise(m, b, innerleftdest, innerleftapex, innerrightorg) >
            0.0) {
            lprevself(*innerleft);
            symself(*innerleft);
        ...
        if (ps_tri_counterclockwise(m, b, innerrightapex, innerrightorg, innerleftdest) >
            0.0) {
            lnextself(*innerright);
            symself(*innerright);
            innerrightorg = innerrightapex;
    ...
    ps_tri_maketriangle(m, b, &baseedge);
    ...
    if (b->verbose > 2) {
        ...
        ps_tri_printtriangle(m, b, &baseedge);
    }
    ...
    while (1) {
        ...
        leftfinished = ps_tri_counterclockwise(m, b, upperleft, lowerleft, lowerright) <=
            0.0;
        rightfinished = ps_tri_counterclockwise(m, b, upperright, lowerleft, lowerright)
            <= 0.0;
        if (leftfinished && rightfinished) {
            /* Create the top new bounding triangle. */
            ps_tri_maketriangle(m, b, &nextedge);
            ...
            if (b->verbose > 2) {
                ...
                ps_tri_printtriangle(m, b, &nextedge);
            }
            /* Special treatment for horizontal cuts. */
            if (b->dwyer && (axis == 1)) {
                org(*farleft, farleftpt);
        ...
        if (!leftfinished) {
            ...
            if (nextapex != (vertex) NULL) {
                ...
                badedge = ps_tri_incircle(m, b, lowerleft, lowerright, upperleft, nextapex) >
                    0.0;
                while (badedge) {
                    ...
                    if (nextapex != (vertex) NULL) {
                        /* Check whether the edge is Delaunay. */
                        badedge = ps_tri_incircle(m, b, lowerleft, lowerright, upperleft,
                                                  nextapex) > 0.0;
                    } else {
        ...
        if (!rightfinished) {
            ...
            if (nextapex != (vertex) NULL) {
                /* Check whether the edge is Delaunay. */
                badedge = ps_tri_incircle(m, b, lowerleft, lowerright, upperright, nextapex) >
                    0.0;
                while (badedge) {
                    /* Eliminate the edge with an edge flip.  As a result, the     */
                    ...
                    if (nextapex != (vertex) NULL) {
                        /* Check whether the edge is Delaunay. */
                        badedge = ps_tri_incircle(m, b, lowerleft, lowerright, upperright,
                                                  nextapex) > 0.0;
                    } else {
                        /* Avoid eating right through the triangulation. */
                        badedge = 0;
        ...
        if (leftfinished || (!rightfinished &&
                             (ps_tri_incircle(m, b, upperleft, lowerleft, lowerright, upperright) >
                              0.0))) {
...
#ifdef ANSI_DECLARATORS
void ps_tri_divconqrecurse(struct mesh *m, struct behavior *b, vertex *sortarray,
                           PS_SIT vertices, PS_SIT axis,
                           struct otri *farleft, struct otri *farright)
#else /* not ANSI_DECLARATORS */
    void ps_tri_divconqrecurse(m, b, sortarray, vertices, axis, farleft, farright)
    struct mesh *m;
    struct behavior *b;
    vertex *sortarray;
    PS_SIT vertices;
...

{
    ...
    if (vertices == 2) {
        ...
        ps_tri_maketriangle(m, b, farleft);
        setorg(*farleft, sortarray[0]);
        setdest(*farleft, sortarray[1]);
        /* The apex is intentionally left NULL. */
        ps_tri_maketriangle(m, b, farright);
        ...
        if (b->verbose > 2) {
            printf("  Creating ");
            ps_tri_printtriangle(m, b, farleft);
            printf("  Creating ");
            ps_tri_printtriangle(m, b, farright);
        }
        ...
        ps_tri_maketriangle(m, b, &midtri);
        ps_tri_maketriangle(m, b, &tri1);
        ps_tri_maketriangle(m, b, &tri2);
        ps_tri_maketriangle(m, b, &tri3);
        area = ps_tri_counterclockwise(m, b, sortarray[0], sortarray[1], sortarray[2]);
        if (area == 0.0) {
        ...
        if (b->verbose > 2) {
            printf("  Creating ");
            ps_tri_printtriangle(m, b, &midtri);
            printf("  Creating ");
            ps_tri_printtriangle(m, b, &tri1);
            printf("  Creating ");
            ps_tri_printtriangle(m, b, &tri2);
            printf("  Creating ");
            ps_tri_printtriangle(m, b, &tri3);
        }
        return;
    } else {
        ...
        ps_tri_divconqrecurse(m, b, sortarray, divider, 1 - axis, farleft, &innerleft);
        ps_tri_divconqrecurse(m, b, &sortarray[divider], vertices - divider, 1 - axis,
                              &innerright, farright);
        if (b->verbose > 1) {
            printf("  Joining triangulations with %lld and %lld vertices.\n", (long long)divider,
                   (long long)(vertices - divider));
        }
        /* Merge the two triangulations into one. */
        ps_tri_mergehulls(m, b, farleft, &innerleft, &innerright, farright, axis);
    }
}

#ifdef ANSI_DECLARATORS
long long ps_tri_removeghosts(struct mesh *m, struct behavior *b, struct otri *startghost)
#else /* not ANSI_DECLARATORS */
    long long ps_tri_removeghosts(m, b, startghost)
    struct mesh *m;
    struct behavior *b;
...

{
    ...
    do {
        ...
        ps_tri_triangledealloc(m, deadtriangle.tri);
...
#ifdef ANSI_DECLARATORS
long long ps_tri_divconqdelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    long long ps_tri_divconqdelaunay(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    sortarray = (vertex *) ps_tri_trimalloc(m->invertices * (PS_SIT) sizeof(vertex));
    ps_tri_traversalinit(&m->vertices);
    for (i = 0; i < m->invertices; i++) {
        sortarray[i] = ps_tri_vertextraverse(m);
    }
    /* Sort the vertices. */
    ps_tri_vertexsort(sortarray, m->invertices);
    ...
    if (b->dwyer) {
        ...
        if (i - divider >= 2) {
            if (divider >= 2) {
                ps_tri_alternateaxes(sortarray, divider, 1);
            }
            ps_tri_alternateaxes(&sortarray[divider], i - divider, 1);
        }
    }
    ...
    ps_tri_divconqrecurse(m, b, sortarray, i, 0, &hullleft, &hullright);
    ps_tri_trifree((VOID *) sortarray);

    return ps_tri_removeghosts(m, b, &hullleft);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_boundingbox(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_boundingbox(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    m->infvertex1 = (vertex) ps_tri_trimalloc(m->vertices.itembytes);
    m->infvertex2 = (vertex) ps_tri_trimalloc(m->vertices.itembytes);
    m->infvertex3 = (vertex) ps_tri_trimalloc(m->vertices.itembytes);
    ...
    ps_tri_maketriangle(m, b, &inftri);
    ...
    if (b->verbose > 2) {
        printf("  Creating ");
        ps_tri_printtriangle(m, b, &inftri);
    }
}
...
#ifdef ANSI_DECLARATORS
long long ps_tri_removebox(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    long long ps_tri_removebox(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_triangledealloc(m, finaledge.tri);

    ps_tri_trifree((VOID *) m->infvertex1);  /* Deallocate the bounding box vertices. */
    ps_tri_trifree((VOID *) m->infvertex2);
    ps_tri_trifree((VOID *) m->infvertex3);
...
#ifdef ANSI_DECLARATORS
long long ps_tri_incrementaldelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    long long ps_tri_incrementaldelaunay(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_boundingbox(m, b);
    if (b->verbose) {
        printf("  Incrementally inserting vertices.\n");
    }
    ps_tri_traversalinit(&m->vertices);
    vertexloop = ps_tri_vertextraverse(m);
    while (vertexloop != (vertex) NULL) {
        starttri.tri = m->dummytri;
        if (ps_tri_insertvertex(m, b, vertexloop, &starttri, (struct osub *) NULL, 0, 0)
            == DUPLICATEVERTEX) {
            if (!b->quiet) {
                printf(
                    "Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored.\n",
...
#ifdef ANSI_DECLARATORS
void ps_tri_eventheapinsert(struct event **heap, PS_SIT heapsize, struct event *newevent)
#else /* not ANSI_DECLARATORS */
    void ps_tri_eventheapinsert(heap, heapsize, newevent)
    struct event **heap;
    PS_SIT heapsize;
...
#ifdef ANSI_DECLARATORS
void ps_tri_eventheapify(struct event **heap, PS_SIT heapsize, PS_SIT eventnum)
#else /* not ANSI_DECLARATORS */
    void ps_tri_eventheapify(heap, heapsize, eventnum)
    struct event **heap;
    PS_SIT heapsize;
    PS_SIT eventnum;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_eventheapdelete(struct event **heap, PS_SIT heapsize, PS_SIT eventnum)
#else /* not ANSI_DECLARATORS */
    void ps_tri_eventheapdelete(heap, heapsize, eventnum)
    struct event **heap;
    PS_SIT heapsize;
    PS_SIT eventnum;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_eventheapify(heap, heapsize - 1, eventnum);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_createeventheap(struct mesh *m, struct event ***eventheap,
                            struct event **events, struct event **freeevents)
#else /* not ANSI_DECLARATORS */
    void ps_tri_createeventheap(m, eventheap, events, freeevents)
    struct mesh *m;
...

{
    ...
    *eventheap = (struct event **) ps_tri_trimalloc(maxevents *
                                                    (PS_SIT) sizeof(struct event *));
    *events = (struct event *) ps_tri_trimalloc(maxevents * (PS_SIT) sizeof(struct event));
    ps_tri_traversalinit(&m->vertices);
    for (i = 0; i < m->invertices; i++) {
        thisvertex = ps_tri_vertextraverse(m);
        (*events)[i].eventptr = (VOID *) thisvertex;
        (*events)[i].xkey = thisvertex[0];
        (*events)[i].ykey = thisvertex[1];
        ps_tri_eventheapinsert(*eventheap, i, *events + i);
    }
...
#ifdef ANSI_DECLARATORS
PS_SIT ps_tri_rightofhyperbola(struct mesh *m, struct otri *fronttri, vertex newsite)
#else /* not ANSI_DECLARATORS */
    PS_SIT ps_tri_rightofhyperbola(m, fronttri, newsite)
    struct mesh *m;
...
#ifdef ANSI_DECLARATORS
REAL ps_tri_circletop(struct mesh *m, vertex pa, vertex pb, vertex pc, REAL ccwabc)
#else /* not ANSI_DECLARATORS */
    REAL ps_tri_circletop(m, pa, pb, pc, ccwabc)
    struct mesh *m;
    vertex pa;
    vertex pb;
    vertex pc;
...
#ifdef ANSI_DECLARATORS
void ps_tri_check4deadevent(struct otri *checktri, struct event **freeevents,
                            struct event **eventheap, PS_SIT *heapsize)
#else /* not ANSI_DECLARATORS */
    void ps_tri_check4deadevent(checktri, freeevents, eventheap, heapsize)
...

{
    ...
    if (eventvertex != (vertex) NULL) {
        ...
        *freeevents = deadevent;
        ps_tri_eventheapdelete(eventheap, *heapsize, eventnum);
        (*heapsize)--;
...
#ifdef ANSI_DECLARATORS
struct splaynode *ps_tri_splay(struct mesh *m, struct splaynode *splaytree,
                               vertex searchpoint, struct otri *searchtri)
#else /* not ANSI_DECLARATORS */
    struct splaynode *ps_tri_splay(m, splaytree, searchpoint, searchtri)
    struct mesh *m;
...

{
    ...
    vertex checkvertex;
    PS_SIT rightofroot, rightofchild;
    ...
    if (checkvertex == splaytree->keydest) {
        rightofroot = ps_tri_rightofhyperbola(m, &splaytree->keyedge, searchpoint);
        if (rightofroot) {
        ...
        if (checkvertex != child->keydest) {
            child = ps_tri_splay(m, child, searchpoint, searchtri);
            if (child == (struct splaynode *) NULL) {
                if (rightofroot) {
        ...
        rightofchild = ps_tri_rightofhyperbola(m, &child->keyedge, searchpoint);
        if (rightofchild) {
            otricopy(child->keyedge, *searchtri);
            grandchild = ps_tri_splay(m, child->rchild, searchpoint, searchtri);
            child->rchild = grandchild;
        } else {
            grandchild = ps_tri_splay(m, child->lchild, searchpoint, searchtri);
        ...
        lefttree = ps_tri_splay(m, splaytree->lchild, searchpoint, searchtri);
        righttree = ps_tri_splay(m, splaytree->rchild, searchpoint, searchtri);

        ps_tri_pooldealloc(&m->splaynodes, (VOID *) splaytree);
...
#ifdef ANSI_DECLARATORS
struct splaynode *ps_tri_splayinsert(struct mesh *m, struct splaynode *splayroot,
                                     struct otri *newkey, vertex searchpoint)
#else /* not ANSI_DECLARATORS */
    struct splaynode *ps_tri_splayinsert(m, splayroot, newkey, searchpoint)
    struct mesh *m;
...

{
    ...
    newsplaynode = (struct splaynode *) ps_tri_poolalloc(&m->splaynodes);
    ...
    } else if (ps_tri_rightofhyperbola(m, &splayroot->keyedge, searchpoint)) {
...
#ifdef ANSI_DECLARATORS
struct splaynode *ps_tri_circletopinsert(struct mesh *m, struct behavior *b,
                                         struct splaynode *splayroot,
                                         struct otri *newkey,
                                         vertex pa, vertex pb, vertex pc, REAL topy)
#else /* not ANSI_DECLARATORS */
    struct splaynode *ps_tri_circletopinsert(m, b, splayroot, newkey, pa, pb, pc, topy)
    struct mesh *m;
    struct behavior *b;
    struct splaynode *splayroot;
    struct otri *newkey;
...

{
    ...
    ccwabc = ps_tri_counterclockwise(m, b, pa, pb, pc);
    xac = pa[0] - pc[0];
    yac = pa[1] - pc[1];
    xbc = pb[0] - pc[0];
    ybc = pb[1] - pc[1];
    ...
    searchpoint[1] = topy;
    return ps_tri_splayinsert(m, ps_tri_splay(m, splayroot, (vertex) searchpoint, &dummytri),
...
#ifdef ANSI_DECLARATORS
struct splaynode *ps_tri_frontlocate(struct mesh *m, struct splaynode *splayroot,
                                     struct otri *bottommost, vertex searchvertex,
                                     struct otri *searchtri, PS_SIT *farright)
#else /* not ANSI_DECLARATORS */
    struct splaynode *ps_tri_frontlocate(m, splayroot, bottommost, searchvertex,
                                         searchtri, farright)
    struct mesh *m;
    struct splaynode *splayroot;
...

{
    ...
    splayroot = ps_tri_splay(m, splayroot, searchvertex, searchtri);

    farrightflag = 0;
    while (!farrightflag && ps_tri_rightofhyperbola(m, searchtri, searchvertex)) {
        onextself(*searchtri);
...
#ifdef ANSI_DECLARATORS
long long ps_tri_sweeplinedelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    long long ps_tri_sweeplinedelaunay(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    PS_SIT heapsize;
    PS_SIT check4events, farrightflag;
    triangle ptr;   /* Temporary variable used by sym(), onext(), and oprev(). */

    ps_tri_poolinit(&m->splaynodes, sizeof(struct splaynode), SPLAYNODEPERBLOCK,
                    SPLAYNODEPERBLOCK, 0);
    ...
    ps_tri_createeventheap(m, &eventheap, &events, &freeevents);
    heapsize = m->invertices;

    if (b->verbose) {
        printf("  Forming triangulation.\n");
    }
    ps_tri_maketriangle(m, b, &lefttri);
    ps_tri_maketriangle(m, b, &righttri);
    ...
    ps_tri_eventheapdelete(eventheap, heapsize, 0);
    heapsize--;
    do {
        if (heapsize == 0) {
    ...
    while (heapsize > 0) {
        ...
        if (nextevent->xkey < m->xmin) {
            ...
            ps_tri_check4deadevent(&farlefttri, &freeevents, eventheap, &heapsize);
            onext(fliptri, farrighttri);
            ps_tri_check4deadevent(&farrighttri, &freeevents, eventheap, &heapsize);

            if (otriequal(farlefttri, bottommost)) {
                lprev(fliptri, bottommost);
            }
            ps_tri_flip(m, b, &fliptri);
            setapex(fliptri, NULL);
            lprev(fliptri, lefttri);
            lnext(fliptri, righttri);
            sym(lefttri, farlefttri);

            if (ps_tri_randomnation(SAMPLERATE) == 0) {
                symself(fliptri);
                dest(fliptri, leftvertex);
                apex(fliptri, midvertex);
                org(fliptri, rightvertex);
                splayroot = ps_tri_circletopinsert(m, b, splayroot, &lefttri, leftvertex,
            ...
            if ((nextvertex[0] == lastvertex[0]) &&
                ...
                splayroot = ps_tri_frontlocate(m, splayroot, &bottommost, nextvertex,
                                               &searchtri, &farrightflag);
/*
  otricopy(bottommost, searchtri);
  farrightflag = 0;
  while (!farrightflag && ps_tri_rightofhyperbola(m, &searchtri, nextvertex)) {
  onextself(searchtri);
  farrightflag = otriequal(searchtri, bottommost);
  }
*/

                ps_tri_check4deadevent(&searchtri, &freeevents, eventheap, &heapsize);
                ...
                if (ps_tri_randomnation(SAMPLERATE) == 0) {
                    splayroot = ps_tri_splayinsert(m, splayroot, &lefttri, nextvertex);
                } else if (ps_tri_randomnation(SAMPLERATE) == 0) {
                    lnext(righttri, inserttri);
                    splayroot = ps_tri_splayinsert(m, splayroot, &inserttri, nextvertex);
                }
            }
        }
        ...
        if (check4events) {
            ...
            lefttest = ps_tri_counterclockwise(m, b, leftvertex, midvertex, rightvertex);
            if (lefttest > 0.0) {
                newevent = freeevents;
                freeevents = (struct event *) freeevents->eventptr;
                newevent->xkey = m->xminextreme;
                newevent->ykey = ps_tri_circletop(m, leftvertex, midvertex, rightvertex,
                                                  lefttest);
                newevent->eventptr = (VOID *) encode(lefttri);
                ps_tri_eventheapinsert(eventheap, heapsize, newevent);
                heapsize++;
            ...
            righttest = ps_tri_counterclockwise(m, b, leftvertex, midvertex, rightvertex);
            if (righttest > 0.0) {
                newevent = freeevents;
                freeevents = (struct event *) freeevents->eventptr;
                newevent->xkey = m->xminextreme;
    ...
    ps_tri_pooldeinit(&m->splaynodes);
    lprevself(bottommost);
    return ps_tri_removeghosts(m, b, &bottommost);
}
...
#ifdef ANSI_DECLARATORS
long long ps_tri_delaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    long long ps_tri_delaunay(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    m->eextras = 0;
    ps_tri_initializetrisubpools(m, b);
    ...
    hulledges = ps_tri_divconqdelaunay(m, b);
#else /* not REDUCED */
    if (!b->quiet) {
    ...
    if (b->incremental) {
        hulledges = ps_tri_incrementaldelaunay(m, b);
    } else if (b->sweepline) {
        hulledges = ps_tri_sweeplinedelaunay(m, b);
    } else {
        hulledges = ps_tri_divconqdelaunay(m, b);
    }
#endif /* not REDUCED */
...
#ifdef ANSI_DECLARATORS
PS_SIT ps_tri_reconstruct(struct mesh *m, struct behavior *b, PS_SIT *trianglelist,
                          REAL *triangleattriblist, REAL *trianglearealist,
                          PS_SIT elements, PS_SIT corners, PS_SIT attribs,
                          PS_SIT *segmentlist,PS_SIT *segmentmarkerlist, PS_SIT numberofsegments)
#else /* not ANSI_DECLARATORS */
    PS_SIT ps_tri_reconstruct(m, b, trianglelist, triangleattriblist, trianglearealist,
    ...
    PS_SIT attribs;
    PS_SIT *segmentlist;
    PS_SIT *segmentmarkerlist;
    PS_SIT numberofsegments;
#endif /* not ANSI_DECLARATORS */

#else /* not TRILIBRARY */

#ifdef ANSI_DECLARATORS
    long long ps_tri_reconstruct(struct mesh *m, struct behavior *b, char *elefilename,
                                 char *areafilename, char *polyfilename, FILE *polyfile)
#else /* not ANSI_DECLARATORS */
        long long ps_tri_reconstruct(m, b, elefilename, areafilename, polyfilename, polyfile)
        struct mesh *m;
        struct behavior *b;
        char *elefilename;
        char *areafilename;
...

    {
        ...
        PS_SIT corner[3];
        PS_SIT end[2];
        PS_SIT killvertexindex;
        PS_SIT incorners;
        PS_SIT segmentmarkers;
        PS_SIT boundmarker;
        PS_SIT aroundvertex;
        long long hullsize;
        PS_SIT notfound;
        ...
        m->inelements = (PS_SIT) strtol(stringptr, &stringptr, 0);
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
            incorners = 3;
        } else {
            incorners = (PS_SIT) strtol(stringptr, &stringptr, 0);
            if (incorners < 3) {
        ...
        if (*stringptr == '\0') {
            ...
            m->eextras = (PS_SIT) strtol(stringptr, &stringptr, 0);
        }
#endif /* not TRILIBRARY */
        ...
        for (elementnumber = 1; elementnumber <= m->inelements; elementnumber++) {
            ps_tri_maketriangle(m, b, &triangleloop);
        ...
        if (b->poly) {
            ...
            segmentmarkers = segmentmarkerlist != (PS_SIT *) NULL;
#else /* not TRILIBRARY */
            ...
            m->insegments = (PS_SIT) strtol(stringptr, &stringptr, 0);
            stringptr = findfield(stringptr);
            if (*stringptr != '\0') {
                segmentmarkers = (PS_SIT) strtol(stringptr, &stringptr, 0);
            }
#endif /* not TRILIBRARY */

            /* Create the subsegments. */
            for (segmentnumber = 1; segmentnumber <= m->insegments; segmentnumber++) {
                ps_tri_makesubseg(m, &subsegloop);
        ...
        if (b->vararea) {
            ...
            areaelements = (PS_SIT) strtol(stringptr, &stringptr, 0);
        ...
        vertexarray = (triangle *) ps_tri_trimalloc(m->vertices.items *
                                                    (PS_SIT) sizeof(triangle));
        ...
        while (triangleloop.tri != (triangle *) NULL) {
            ...
            for (j = 0; j < 3; j++) {
                stringptr = findfield(stringptr);
                if (*stringptr == '\0') {
                    printf("Error:  Triangle %lld is missing vertex %d in %s.\n",
                           elementnumber, j + 1, elefilename);
                    ps_tri_triexit(1);
                } else {
                    corner[j] = (PS_SIT) strtol(stringptr, &stringptr, 0);
                    if ((corner[j] < b->firstnumber) ||
                        (corner[j] >= b->firstnumber + m->invertices)) {
                        printf("Error:  Triangle %lld has an invalid vertex index.\n",
                               elementnumber);
            ...
            for (j = 3; j < incorners; j++) {
                ...
                stringptr = findfield(stringptr);
                if (*stringptr != '\0') {
                    killvertexindex = (PS_SIT) strtol(stringptr, &stringptr, 0);
#endif /* not TRILIBRARY */
                    if ((killvertexindex >= b->firstnumber) &&
                        ...
                        killvertex = ps_tri_getvertex(m, b, killvertexindex);
                        if (vertextype(killvertex) != DEADVERTEX) {
                            ps_tri_vertexdealloc(m, killvertex);
                        }
                    }
#ifndef TRILIBRARY
                }
      ...
      /* Read an area constraint from the .area file. */
            ...
            setorg(triangleloop, ps_tri_getvertex(m, b, corner[0]));
            setdest(triangleloop, ps_tri_getvertex(m, b, corner[1]));
            setapex(triangleloop, ps_tri_getvertex(m, b, corner[2]));
        ...
        if (b->poly) {
            ...
            boundmarker = 0;
            ps_tri_traversalinit(&m->subsegs);
            subsegloop.ss = ps_tri_subsegtraverse(m);
      ...
      /* Read the endpoints of each segment, and possibly a boundary marker. */
                ...
                stringptr = findfield(stringptr);
                if (*stringptr == '\0') {
                    printf("Error:  Segment %lld has no endpoints in %s.\n", segmentnumber,
                           polyfilename);
                    ps_tri_triexit(1);
                } else {
                    end[0] = (PS_SIT) strtol(stringptr, &stringptr, 0);
                }
                stringptr = findfield(stringptr);
                if (*stringptr == '\0') {
                    printf("Error:  Segment %lld is missing its second endpoint in %s.\n",
                           segmentnumber, polyfilename);
                    ps_tri_triexit(1);
                } else {
                    end[1] = (PS_SIT) strtol(stringptr, &stringptr, 0);
                }
                if (segmentmarkers) {
                    stringptr = findfield(stringptr);
                    if (*stringptr == '\0') {
                        ...
                        boundmarker = (PS_SIT) strtol(stringptr, &stringptr, 0);
                    }
                }
#endif /* not TRILIBRARY */
                for (j = 0; j < 2; j++) {
                ...
                segmentorg = ps_tri_getvertex(m, b, end[0]);
                segmentdest = ps_tri_getvertex(m, b, end[1]);
                        ...
                        if (shorg == checkdest) {
                            ...
                            *prevlink = checktri.tri[6 + checktri.orient];
                            ...
                            if (checkneighbor.tri == m->dummytri) {
                                ...
                                ps_tri_insertsubseg(m, b, &checktri, 1);
                                hullsize++;
                            }
                            notfound = 0;
                        }
        ...
        ps_tri_trifree((VOID *) vertexarray);
        return hullsize;
    }
...
#ifdef ANSI_DECLARATORS
enum finddirectionresult ps_tri_finddirection(struct mesh *m, struct behavior *b,
                                              struct otri *searchtri,
                                              vertex searchpoint)
#else /* not ANSI_DECLARATORS */
    enum finddirectionresult ps_tri_finddirection(m, b, searchtri, searchpoint)
    struct mesh *m;
    struct behavior *b;
    struct otri *searchtri;
    vertex searchpoint;
...

{
    ...
    leftccw = ps_tri_counterclockwise(m, b, searchpoint, startvertex, leftvertex);
    leftflag = leftccw > 0.0;
    /* Is `searchpoint' to the right? */
    rightccw = ps_tri_counterclockwise(m, b, startvertex, searchpoint, rightvertex);
...
#ifdef ANSI_DECLARATORS
void ps_tri_segmentintersection(struct mesh *m, struct behavior *b,
                                struct otri *splittri, struct osub *splitsubseg,
                                vertex endpoint2)
#else /* not ANSI_DECLARATORS */
    void ps_tri_segmentintersection(m, b, splittri, splitsubseg, endpoint2)
    struct mesh *m;
    struct behavior *b;
...

{
    ...
    newvertex = (vertex) ps_tri_poolalloc(&m->vertices);
    ...
    success = ps_tri_insertvertex(m, b, newvertex, splittri, splitsubseg, 0, 0);
    ...
    collinear = ps_tri_finddirection(m, b, splittri, endpoint1);
...
#ifdef ANSI_DECLARATORS
PS_SIT ps_tri_scoutsegment(struct mesh *m, struct behavior *b, struct otri *searchtri,
                           vertex endpoint2, PS_SIT newmark)
#else /* not ANSI_DECLARATORS */
    PS_SIT ps_tri_scoutsegment(m, b, searchtri, endpoint2, newmark)
    struct mesh *m;
    struct behavior *b;
    struct otri *searchtri;
    vertex endpoint2;
...

{
    ...
    collinear = ps_tri_finddirection(m, b, searchtri, endpoint2);
    dest(*searchtri, rightvertex);
    apex(*searchtri, leftvertex);
    if (((leftvertex[0] == endpoint2[0]) && (leftvertex[1] == endpoint2[1])) ||
        ...
        ps_tri_insertsubseg(m, b, searchtri, newmark);
        return 1;
        ...
        return ps_tri_scoutsegment(m, b, searchtri, endpoint2, newmark);
        ...
        if (crosssubseg.ss == m->dummysub) {
            ...
            ps_tri_segmentintersection(m, b, &crosstri, &crosssubseg, endpoint2);
...
#ifdef ANSI_DECLARATORS
void ps_tri_conformingedge(struct mesh *m, struct behavior *b,
                           vertex endpoint1, vertex endpoint2, PS_SIT newmark)
#else /* not ANSI_DECLARATORS */
    void ps_tri_conformingedge(m, b, endpoint1, endpoint2, newmark)
    struct mesh *m;
    struct behavior *b;
    vertex endpoint1;
    vertex endpoint2;
...

{
    ...
    success = ps_tri_insertvertex(m, b, newvertex, &searchtri1, (struct osub *) NULL,
                                  0, 0);
    if (success == DUPLICATEVERTEX) {
        ...
        ps_tri_vertexdealloc(m, newvertex);
        ...
        if (success == VIOLATINGVERTEX) {
            ...
            success = ps_tri_insertvertex(m, b, newvertex, &searchtri1, &brokensubseg,
                                          0, 0);
            if (success != SUCCESSFULVERTEX) {
    ...
    /*   segment.                                                             */
    ps_tri_finddirection(m, b, &searchtri2, endpoint2);
    if (!ps_tri_scoutsegment(m, b, &searchtri1, endpoint1, newmark)) {
        /* The origin of searchtri1 may have changed if a collision with an */
        /*   intervening vertex on the segment occurred.                    */
        org(searchtri1, midvertex1);
        ps_tri_conformingedge(m, b, midvertex1, endpoint1, newmark);
    }
    if (!ps_tri_scoutsegment(m, b, &searchtri2, endpoint2, newmark)) {
        /* The origin of searchtri2 may have changed if a collision with an */
        /*   intervening vertex on the segment occurred.                    */
        org(searchtri2, midvertex2);
        ps_tri_conformingedge(m, b, midvertex2, endpoint2, newmark);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_delaunayfixup(struct mesh *m, struct behavior *b,
                          struct otri *fixuptri, PS_SIT leftside)
#else /* not ANSI_DECLARATORS */
    void ps_tri_delaunayfixup(m, b, fixuptri, leftside)
    struct mesh *m;
    struct behavior *b;
...

{
    ...
    if (leftside) {
        if (ps_tri_counterclockwise(m, b, nearvertex, leftvertex, farvertex) <= 0.0) {
            /* leftvertex is a reflex vertex too.  Nothing can */
            /*   be done until a convex section is found.      */
            return;
        }
    } else {
        if (ps_tri_counterclockwise(m, b, farvertex, rightvertex, nearvertex) <= 0.0) {
            /* rightvertex is a reflex vertex too.  Nothing can */
            /*   be done until a convex section is found.       */
            return;
        }
    }
    if (ps_tri_counterclockwise(m, b, rightvertex, leftvertex, farvertex) > 0.0) {
        /* fartri is not an inverted triangle, and farvertex is not a reflex */
        /*   vertex.  As there are no reflex vertices, fixuptri isn't an     */
        /*   inverted triangle, either.  Hence, test the edge between the    */
        /*   triangles to ensure it is locally Delaunay.                     */
        if (ps_tri_incircle(m, b, leftvertex, farvertex, rightvertex, nearvertex) <=
            0.0) {
            return;
        }
        /* Not locally Delaunay; go on to an edge flip. */
    }        /* else fartri is inverted; remove it from the stack by flipping. */
    ps_tri_flip(m, b, &neartri);
    lprevself(*fixuptri);    /* Restore the origin of fixuptri after the flip. */
    /* Recursively process the two triangles that result from the flip. */
    ps_tri_delaunayfixup(m, b, fixuptri, leftside);
    ps_tri_delaunayfixup(m, b, &fartri, leftside);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_constrainededge(struct mesh *m, struct behavior *b,
                            struct otri *starttri, vertex endpoint2, PS_SIT newmark)
#else /* not ANSI_DECLARATORS */
    void ps_tri_constrainededge(m, b, starttri, endpoint2, newmark)
    struct mesh *m;
    struct behavior *b;
    struct otri *starttri;
    vertex endpoint2;
...

{
    ...
    do {
        ...
        if ((farvertex[0] == endpoint2[0]) && (farvertex[1] == endpoint2[1])) {
            ...
            ps_tri_delaunayfixup(m, b, &fixuptri, 0);
            ps_tri_delaunayfixup(m, b, &fixuptri2, 1);
            done = 1;
        } else {
            /* Check whether farvertex is to the left or right of the segment */
            /*   being inserted, to decide which edge of fixuptri to dig      */
            /*   through next.                                                */
            area = ps_tri_counterclockwise(m, b, endpoint1, endpoint2, farvertex);
            if (area == 0.0) {
                ...
                if (crosssubseg.ss == m->dummysub) {
                    ps_tri_flip(m, b, &fixuptri);    /* May create inverted triangle at left. */
                } else {
                    /* We've collided with a segment between endpoint1 and endpoint2. */
                    collision = 1;
                    /* Insert a vertex at the intersection. */
                    ps_tri_segmentintersection(m, b, &fixuptri, &crosssubseg, endpoint2);
                    done = 1;
                }
            }
        }
    ...
    ps_tri_insertsubseg(m, b, &fixuptri, newmark);
    /* If there was a collision with an interceding vertex, install another */
    /*   segment connecting that vertex with endpoint2.                     */
    if (collision) {
        /* Insert the remainder of the segment. */
        if (!ps_tri_scoutsegment(m, b, &fixuptri, endpoint2, newmark)) {
            ps_tri_constrainededge(m, b, &fixuptri, endpoint2, newmark);
        }
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_insertsegment(struct mesh *m, struct behavior *b,
                          vertex endpoint1, vertex endpoint2, PS_SIT newmark)
#else /* not ANSI_DECLARATORS */
    void ps_tri_insertsegment(m, b, endpoint1, endpoint2, newmark)
    struct mesh *m;
    struct behavior *b;
    vertex endpoint1;
    vertex endpoint2;
...

{
    ...
    if (checkvertex != endpoint1) {
        ...
        if (ps_tri_locate(m, b, endpoint1, &searchtri1) != ONVERTEX) {
            printf(
    ...
    /*   toward the second.                                   */
    if (ps_tri_scoutsegment(m, b, &searchtri1, endpoint2, newmark)) {
    ...
    if (checkvertex != endpoint2) {
        ...
        if (ps_tri_locate(m, b, endpoint2, &searchtri2) != ONVERTEX) {
            printf(
                "Internal error in insertsegment():  Unable to locate PSLG vertex\n");
            printf("  (%.12g, %.12g) in triangulation.\n",
                   endpoint2[0], endpoint2[1]);
    ...
    /*   toward the first.                                     */
    if (ps_tri_scoutsegment(m, b, &searchtri2, endpoint1, newmark)) {
        /* The segment was easily inserted. */
        return;
    }
    ...
    if (b->splitseg) {
        ...
        ps_tri_conformingedge(m, b, endpoint1, endpoint2, newmark);
    } else {
#endif /* not CDT_ONLY */
#endif /* not REDUCED */
        /* Insert the segment directly into the triangulation. */
        ps_tri_constrainededge(m, b, &searchtri1, endpoint2, newmark);
#ifndef REDUCED
#ifndef CDT_ONLY
    }
#endif /* not CDT_ONLY */
...
#ifdef ANSI_DECLARATORS
void ps_tri_markhull(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_markhull(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    do {
        ...
        ps_tri_insertsubseg(m, b, &hulltri, 1);
...
#ifdef ANSI_DECLARATORS
void ps_tri_formskeleton(struct mesh *m, struct behavior *b, PS_SIT *segmentlist,
                         PS_SIT *segmentmarkerlist, PS_SIT numberofsegments)
#else /* not ANSI_DECLARATORS */
    void ps_tri_formskeleton(m, b, segmentlist, segmentmarkerlist, numberofsegments)
    struct mesh *m;
    struct behavior *b;
    PS_SIT *segmentlist;
    PS_SIT *segmentmarkerlist;
...
#ifdef ANSI_DECLARATORS
    void ps_tri_formskeleton(struct mesh *m, struct behavior *b,
                             FILE *polyfile, char *polyfilename)
#else /* not ANSI_DECLARATORS */
        void ps_tri_formskeleton(m, b, polyfile, polyfilename)
        struct mesh *m;
        struct behavior *b;
        FILE *polyfile;
        char *polyfilename;
        ...
        if (b->poly) {
            ...
            for (i = 0; i < m->insegments; i++) {
                ...
                if (*stringptr == '\0') {
                    ...
                    end1 = (PS_SIT) strtol(stringptr, &stringptr, 0);
                }
                stringptr = findfield(stringptr);
                if (*stringptr == '\0') {
                    ...
                    end2 = (PS_SIT) strtol(stringptr, &stringptr, 0);
                }
                if (segmentmarkers) {
                    stringptr = findfield(stringptr);
                    if (*stringptr == '\0') {
                ...
                if ((end1 < b->firstnumber) ||
                    ...
                    if (!b->quiet) {
                        printf("Warning:  Invalid first endpoint of segment %lld in %s.\n",
                               (long long)b->firstnumber + i, polyfilename);
                    }
                    ...
                    endpoint1 = ps_tri_getvertex(m, b, end1);
                    endpoint2 = ps_tri_getvertex(m, b, end2);
                    if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
                        ...
                        ps_tri_insertsegment(m, b, endpoint1, endpoint2, boundmarker);
                    }
                }
            }
        } else {
...
#ifdef ANSI_DECLARATORS
void ps_tri_infecthull(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_infecthull(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */
    ...
    do {
        ...
        if (!infected(hulltri)) {
            ...
            if (hullsubseg.ss == m->dummysub) {
                ...
                if (!infected(hulltri)) {
                    infect(hulltri);
                    deadtriangle = (triangle **) ps_tri_poolalloc(&m->viri);
...
#ifdef ANSI_DECLARATORS
void ps_tri_plague(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_plague(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_traversalinit(&m->viri);
    virusloop = (triangle **) ps_tri_traverse(&m->viri);
        ...
        for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
            ...
            if ((neighbor.tri == m->dummytri) || infected(neighbor)) {
                if (neighborsubseg.ss != m->dummysub) {
                    ...
                    /*   dies too.                                                 */
                    ps_tri_subsegdealloc(m, neighborsubseg.ss);
    ...
    while (virusloop != (triangle **) NULL) {
        ...
        ps_tri_triangledealloc(m, testtri.tri);
        virusloop = (triangle **) ps_tri_traverse(&m->viri);
    }
    /* Empty the virus pool. */
    ps_tri_poolrestart(&m->viri);
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_regionplague(struct mesh *m, struct behavior *b,
                         REAL attribute, REAL area)
#else /* not ANSI_DECLARATORS */
    void ps_tri_regionplague(m, b, attribute, area)
    struct mesh *m;
    struct behavior *b;
    REAL attribute;
    REAL area;
    ...
    while (virusloop != (triangle **) NULL) {
        ...
        for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
            ...
            if ((neighbor.tri != m->dummytri) && !infected(neighbor)
                ...
                infect(neighbor);
                /* Ensure that the neighbor's neighbors will be infected. */
                regiontri = (triangle **) ps_tri_poolalloc(&m->viri);
...
#ifdef ANSI_DECLARATORS
void ps_tri_carveholes(struct mesh *m, struct behavior *b, REAL *holelist, PS_SIT holes,
                       REAL *regionlist, PS_SIT regions)
#else /* not ANSI_DECLARATORS */
    void ps_tri_carveholes(m, b, holelist, holes, regionlist, regions)
    struct mesh *m;
    struct behavior *b;
    REAL *holelist;
    PS_SIT holes;
...

{
    ...
    if (regions > 0) {
        ...
        regiontris = (struct otri *) ps_tri_trimalloc(regions *
                                                      (PS_SIT) sizeof(struct otri));
    } else {
    ...
    if (((holes > 0) && !b->noholes) || !b->convex || (regions > 0)) {
        ...
        ps_tri_poolinit(&m->viri, sizeof(triangle *), VIRUSPERBLOCK, VIRUSPERBLOCK, 0);
    }

    if (!b->convex) {
        /* Mark as infected any unprotected triangles on the boundary. */
        /*   This is one way by which concavities are created.         */
        ps_tri_infecthull(m, b);
    }
    ...
    if ((holes > 0) && !b->noholes) {
        ...
        for (i = 0; i < 2 * holes; i += 2) {
            ...
            if ((holelist[i] >= m->xmin) && (holelist[i] <= m->xmax)
                ...
                if (ps_tri_counterclockwise(m, b, searchorg, searchdest, &holelist[i]) >
                    0.0) {
                    /* Find a triangle that contains the hole. */
                    intersect = ps_tri_locate(m, b, &holelist[i], &searchtri);
                    if ((intersect != OUTSIDE) && (!infected(searchtri))) {
                        /* Infect the triangle.  This is done by marking the triangle  */
                        /*   as infected and including the triangle in the virus pool. */
                        infect(searchtri);
                        holetri = (triangle **) ps_tri_poolalloc(&m->viri);
    ...
    if (regions > 0) {
        ...
        for (i = 0; i < regions; i++) {
            ...
            if ((regionlist[4 * i] >= m->xmin) && (regionlist[4 * i] <= m->xmax) &&
                ...
                org(searchtri, searchorg);
                dest(searchtri, searchdest);
                if (ps_tri_counterclockwise(m, b, searchorg, searchdest, &regionlist[4 * i]) >
                    0.0) {
                    /* Find a triangle that contains the region point. */
                    intersect = ps_tri_locate(m, b, &regionlist[4 * i], &searchtri);
                    if ((intersect != OUTSIDE) && (!infected(searchtri))) {
            ...
            if (regiontris[i].tri != m->dummytri) {
                ...
                if (!deadtri(regiontris[i].tri)) {
                    ...
                    *regiontri = regiontris[i].tri;
                    ...
                    ps_tri_regionplague(m, b, regionlist[4 * i + 2], regionlist[4 * i + 3]);
                    /* The virus pool should be empty now. */
                }
            }
        }
    ...
    /* Free up memory. */
    if (((holes > 0) && !b->noholes) || !b->convex || (regions > 0)) {
        ps_tri_pooldeinit(&m->viri);
    }
    if (regions > 0) {
        ps_tri_trifree((VOID *) regiontris);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_tallyencs(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_tallyencs(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    while (subsegloop.ss != (subseg *) NULL) {
        ...
        dummy = ps_tri_checkseg4encroach(m, b, &subsegloop);
        subsegloop.ss = ps_tri_subsegtraverse(m);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_splitencsegs(struct mesh *m, struct behavior *b, PS_SIT triflaws)
#else /* not ANSI_DECLARATORS */
    void ps_tri_splitencsegs(m, b, triflaws)
    struct mesh *m;
    struct behavior *b;
    PS_SIT triflaws;
#endif /* not ANSI_DECLARATORS */

{
    ...
    PS_SIT acuteorg, acuteorg2, acutedest, acutedest2;
    PS_SIT dummy;
    PS_SIT i;
    triangle ptr;                     /* Temporary variable used by stpivot(). */
    subseg sptr;                        /* Temporary variable used by snext(). */
    ...
    while ((m->badsubsegs.items > 0) && (m->steinerleft != 0)) {
        ps_tri_traversalinit(&m->badsubsegs);
        encloop = ps_tri_badsubsegtraverse(m);
            ...
            if (!deadsubseg(currentenc.ss) &&
                ...
                if (!b->conformdel && !acuteorg && !acutedest) {
                    ...
                    while ((vertextype(eapex) == FREEVERTEX) &&
                        ...
                        ps_tri_deletevertex(m, b, &testtri);
                        stpivot(currentenc, enctri);
                        apex(enctri, eapex);
                ...
                if (!b->noexact) {
                    ...
                    multiplier = ps_tri_counterclockwise(m, b, eorg, edest, newvertex);
                ...
                if (((newvertex[0] == eorg[0]) && (newvertex[1] == eorg[1])) ||
                    ...
                    ps_tri_precisionerror();
                    ps_tri_triexit(1);
                }
                /* Insert the splitting vertex.  This should always succeed. */
                success = ps_tri_insertvertex(m, b, newvertex, &enctri, &currentenc,
                                              1, triflaws);
                ...
                dummy = ps_tri_checkseg4encroach(m, b, &currentenc);
                snextself(currentenc);
                dummy = ps_tri_checkseg4encroach(m, b, &currentenc);
            }

            ps_tri_badsubsegdealloc(m, encloop);
            encloop = ps_tri_badsubsegtraverse(m);
        }
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_tallyfaces(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_tallyfaces(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    while (triangleloop.tri != (triangle *) NULL) {
        ...
        ps_tri_testtriangle(m, b, &triangleloop);
        triangleloop.tri = ps_tri_triangletraverse(m);
    }
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_splittriangle(struct mesh *m, struct behavior *b,
                          struct badtriang *badtri)
#else /* not ANSI_DECLARATORS */
    void ps_tri_splittriangle(m, b, badtri)
    struct mesh *m;
    struct behavior *b;
    struct badtriang *badtri;
#endif /* not ANSI_DECLARATORS */

{
    ...
    if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
        ...
        newvertex = (vertex) ps_tri_poolalloc(&m->vertices);
        ps_tri_findcircumcenter(m, b, borg, bdest, bapex, newvertex, &xi, &eta, 1);
        ...
        if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) ||
            ...
            success = ps_tri_insertvertex(m, b, newvertex, &badotri, (struct osub *) NULL,
                                          1, 1);
            if (success == SUCCESSFULVERTEX) {
                if (m->steinerleft > 0) {
                    m->steinerleft--;
...
#ifdef ANSI_DECLARATORS
void ps_tri_enforcequality(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_enforcequality(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    ps_tri_poolinit(&m->badsubsegs, sizeof(struct badsubseg), BADSUBSEGPERBLOCK,
                    BADSUBSEGPERBLOCK, 0);
    if (b->verbose) {
        printf("  Looking for encroached subsegments.\n");
    }
    /* Test all segments to see if they're encroached. */
    ps_tri_tallyencs(m, b);
    ...
    if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
        ...
        ps_tri_poolinit(&m->badtriangles, sizeof(struct badtriang), BADTRIPERBLOCK,
                        BADTRIPERBLOCK, 0);
        ...
        ps_tri_tallyfaces(m, b);
        /* Initialize the pool of recently flipped triangles. */
        ps_tri_poolinit(&m->flipstackers, sizeof(struct flipstacker), FLIPSTACKERPERBLOCK,
        ...
        while ((m->badtriangles.items > 0) && (m->steinerleft != 0)) {
            ...
            badtri = ps_tri_dequeuebadtriang(m);
            ps_tri_splittriangle(m, b, badtri);
            if (m->badsubsegs.items > 0) {
                /* Put bad triangle back in queue for another try later. */
                ps_tri_enqueuebadtriang(m, b, badtri);
                /* Fix any encroached subsegments that resulted. */
                /*   Record any new bad triangles that result.   */
                ps_tri_splitencsegs(m, b, 1);
            } else {
                /* Return the bad triangle to the pool. */
                ps_tri_pooldealloc(&m->badtriangles, (VOID *) badtri);
            }
        }
    }
...
#ifdef ANSI_DECLARATORS
void ps_tri_highorder(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_highorder(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
char *ps_tri_readline(char *string, FILE *infile, char *infilename)
#else /* not ANSI_DECLARATORS */
    char *ps_tri_readline(string, infile, infilename)
    char *string;
    FILE *infile;
    char *infilename;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
char *ps_tri_findfield(char *string)
#else /* not ANSI_DECLARATORS */
    char *ps_tri_findfield(string)
    char *string;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_readnodes(struct mesh *m, struct behavior *b, char *nodefilename,
                      char *polyfilename, FILE **polyfile)
#else /* not ANSI_DECLARATORS */
    void ps_tri_readnodes(m, b, nodefilename, polyfilename, polyfile)
    struct mesh *m;
    struct behavior *b;
    char *nodefilename;
    char *polyfilename;
...

{
    ...
    if (b->poly) {
        ...
        *polyfile = fopen(polyfilename, "r");
        ...
        stringptr = ps_tri_readline(inputline, *polyfile, polyfilename);
        m->invertices = (PS_SIT) strtol(stringptr, &stringptr, 0);
        stringptr = ps_tri_findfield(stringptr);
        if (*stringptr == '\0') {
            m->mesh_dim = 2;
        } else {
            m->mesh_dim = (PS_SIT) strtol(stringptr, &stringptr, 0);
        }
        stringptr = ps_tri_findfield(stringptr);
        if (*stringptr == '\0') {
            m->nextras = 0;
        } else {
            m->nextras = (PS_SIT) strtol(stringptr, &stringptr, 0);
        }
        stringptr = ps_tri_findfield(stringptr);
        if (*stringptr == '\0') {
            nodemarkers = 0;
        } else {
            nodemarkers = (PS_SIT) strtol(stringptr, &stringptr, 0);
        }
        if (m->invertices > 0) {
            infile = *polyfile;
    ...
    if (m->readnodefile) {
        ...
        /* Read number of vertices, number of dimensions, number of vertex */
        /*   attributes, and number of boundary markers.                   */
        stringptr = ps_tri_readline(inputline, infile, nodefilename);
        m->invertices = (PS_SIT) strtol(stringptr, &stringptr, 0);
        stringptr = ps_tri_findfield(stringptr);
        if (*stringptr == '\0') {
            m->mesh_dim = 2;
    ...
    ps_tri_initializevertexpool(m, b);

    /* Read the vertices. */
    for (i = 0; i < m->invertices; i++) {
        vertexloop = (vertex) ps_tri_poolalloc(&m->vertices);
        stringptr = ps_tri_readline(inputline, infile, infilename);
        if (i == 0) {
            firstnode = (PS_SIT) strtol(stringptr, &stringptr, 0);
        ...
        if (nodemarkers) {
            ...
            if (*stringptr == '\0') {
                ...
                currentmarker = (PS_SIT) strtol(stringptr, &stringptr, 0);
...
#ifdef ANSI_DECLARATORS
void ps_tri_transfernodes(struct mesh *m, struct behavior *b, REAL *pointlist,
                          REAL *pointattriblist, PS_SIT *pointmarkerlist,
                          PS_SIT numberofpoints, PS_SIT numberofpointattribs)
#else /* not ANSI_DECLARATORS */
    void ps_tri_transfernodes(m, b, pointlist, pointattriblist, pointmarkerlist,
                              numberofpoints, numberofpointattribs)
    struct mesh *m;
    struct behavior *b;
    REAL *pointlist;
    REAL *pointattriblist;
    PS_SIT *pointmarkerlist;
    PS_SIT numberofpoints;
    PS_SIT numberofpointattribs;
#endif /* not ANSI_DECLARATORS */

{
    ...
    for (i = 0; i < m->invertices; i++) {
        ...
        if (pointmarkerlist != (PS_SIT *) NULL) {
            /* Read a vertex marker. */
...
#ifdef ANSI_DECLARATORS
void ps_tri_readholes(struct mesh *m, struct behavior *b,
                      FILE *polyfile, char *polyfilename, REAL **hlist, PS_SIT *holes,
                      REAL **rlist, PS_SIT *regions)
#else /* not ANSI_DECLARATORS */
    void ps_tri_readholes(m, b, polyfile, polyfilename, hlist, holes, rlist, regions)
    struct mesh *m;
    struct behavior *b;
    FILE *polyfile;
    char *polyfilename;
...

{
    ...
    /* Read the holes. */
    stringptr = ps_tri_readline(inputline, polyfile, polyfilename);
    *holes = (PS_SIT) strtol(stringptr, &stringptr, 0);
    if (*holes > 0) {
        holelist = (REAL *) ps_tri_trimalloc(2 * *holes * (PS_SIT) sizeof(REAL));
        *hlist = holelist;
        for (i = 0; i < 2 * *holes; i += 2) {
            stringptr = ps_tri_readline(inputline, polyfile, polyfilename);
            stringptr = ps_tri_findfield(stringptr);
    ...
    if ((b->regionattrib || b->vararea) && !b->refine) {
        ...
        stringptr = ps_tri_readline(inputline, polyfile, polyfilename);
        *regions = (PS_SIT) strtol(stringptr, &stringptr, 0);
        if (*regions > 0) {
            regionlist = (REAL *) ps_tri_trimalloc(4 * *regions * (PS_SIT) sizeof(REAL));
            *rlist = regionlist;
            index = 0;
            for (i = 0; i < *regions; i++) {
                stringptr = ps_tri_readline(inputline, polyfile, polyfilename);
...
#ifdef ANSI_DECLARATORS
void ps_tri_finishfile(FILE *outfile, PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
    void ps_tri_finishfile(outfile, argc, argv)
    FILE *outfile;
    PS_SIT argc;
    char **argv;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_writenodes(struct mesh *m, struct behavior *b, REAL **pointlist,
                       REAL **pointattriblist, PS_SIT **pointmarkerlist)
#else /* not ANSI_DECLARATORS */
    void ps_tri_writenodes(m, b, pointlist, pointattriblist, pointmarkerlist)
    struct mesh *m;
    struct behavior *b;
    REAL **pointlist;
    REAL **pointattriblist;
...
#ifdef ANSI_DECLARATORS
    void ps_tri_writenodes(struct mesh *m, struct behavior *b, char *nodefilename,
                           PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
        void ps_tri_writenodes(m, b, nodefilename, argc, argv)
        struct mesh *m;
        struct behavior *b;
        char *nodefilename;
        PS_SIT argc;
...

    {
        ...
        if (*pointlist == (REAL *) NULL) {
            *pointlist = (REAL *) ps_tri_trimalloc((PS_SIT) (outvertices * 2 * sizeof(REAL)));
        }
        /* Allocate memory for output vertex attributes if necessary. */
        if ((m->nextras > 0) && (*pointattriblist == (REAL *) NULL)) {
            *pointattriblist = (REAL *) ps_tri_trimalloc((PS_SIT) (outvertices * m->nextras *
                                                                   sizeof(REAL)));
        }
        /* Allocate memory for output vertex markers if necessary. */
        if (!b->nobound && (*pointmarkerlist == (PS_SIT *) NULL)) {
            *pointmarkerlist = (PS_SIT *) ps_tri_trimalloc((PS_SIT) (outvertices * sizeof(PS_SIT)));
        }
        plist = *pointlist;
...
#ifndef TRILIBRARY
        ps_tri_finishfile(outfile, argc, argv);
#endif /* not TRILIBRARY */
    }
...
#ifdef ANSI_DECLARATORS
void ps_tri_numbernodes(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_numbernodes(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_writeelements(struct mesh *m, struct behavior *b,
                          PS_SIT **trianglelist, REAL **triangleattriblist)
#else /* not ANSI_DECLARATORS */
    void ps_tri_writeelements(m, b, trianglelist, triangleattriblist)
    struct mesh *m;
    struct behavior *b;
    PS_SIT **trianglelist;
    REAL **triangleattriblist;
...
#ifdef ANSI_DECLARATORS
    void ps_tri_writeelements(struct mesh *m, struct behavior *b, char *elefilename,
                              PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
        void ps_tri_writeelements(m, b, elefilename, argc, argv)
        struct mesh *m;
        struct behavior *b;
        char *elefilename;
        PS_SIT argc;
...

    {
        ...
        if (*trianglelist == (PS_SIT *) NULL) {
            *trianglelist = (PS_SIT *) ps_tri_trimalloc((PS_SIT) (m->triangles.items *
                                                                  ((b->order + 1) * (b->order + 2) /
                                                                   2) * sizeof(PS_SIT)));
        }
        /* Allocate memory for output triangle attributes if necessary. */
        if ((m->eextras > 0) && (*triangleattriblist == (REAL *) NULL)) {
            *triangleattriblist = (REAL *) ps_tri_trimalloc((PS_SIT) (m->triangles.items *
                                                                      m->eextras *
                                                                      sizeof(REAL)));
        }
        tlist = *trianglelist;
...
#ifdef ANSI_DECLARATORS
void ps_tri_writepoly(struct mesh *m, struct behavior *b,
                      PS_SIT **segmentlist, PS_SIT **segmentmarkerlist)
#else /* not ANSI_DECLARATORS */
    void ps_tri_writepoly(m, b, segmentlist, segmentmarkerlist)
    struct mesh *m;
    struct behavior *b;
    PS_SIT **segmentlist;
    PS_SIT **segmentmarkerlist;
...
#ifdef ANSI_DECLARATORS
    void ps_tri_writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
                          REAL *holelist, PS_SIT holes, REAL *regionlist, PS_SIT regions,
                          PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
        void ps_tri_writepoly(m, b, polyfilename, holelist, holes, regionlist, regions,
                              argc, argv)
        struct mesh *m;
        struct behavior *b;
        char *polyfilename;
...

    {
        ...
        if (*segmentlist == (PS_SIT *) NULL) {
            *segmentlist = (PS_SIT *) ps_tri_trimalloc((PS_SIT) (m->subsegs.items * 2 *
                                                                 sizeof(PS_SIT)));
        }
        /* Allocate memory for output segment markers if necessary. */
        if (!b->nobound && (*segmentmarkerlist == (PS_SIT *) NULL)) {
            *segmentmarkerlist = (PS_SIT *) ps_tri_trimalloc((PS_SIT) (m->subsegs.items *
                                                                       sizeof(PS_SIT)));
        }
        slist = *segmentlist;
...
#ifdef ANSI_DECLARATORS
void ps_tri_writeedges(struct mesh *m, struct behavior *b,
                       PS_SIT **edgelist, PS_SIT **edgemarkerlist)
#else /* not ANSI_DECLARATORS */
    void ps_tri_writeedges(m, b, edgelist, edgemarkerlist)
    struct mesh *m;
    struct behavior *b;
    PS_SIT **edgelist;
    PS_SIT **edgemarkerlist;
#endif /* not ANSI_DECLARATORS */

#else /* not TRILIBRARY */

#ifdef ANSI_DECLARATORS
    void ps_tri_writeedges(struct mesh *m, struct behavior *b, char *edgefilename,
                           PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
        void ps_tri_writeedges(m, b, edgefilename, argc, argv)
        struct mesh *m;
        struct behavior *b;
        char *edgefilename;
        PS_SIT argc;
...

    {
        ...
        if (*edgelist == (PS_SIT *) NULL) {
            *edgelist = (PS_SIT *) ps_tri_trimalloc((PS_SIT) (m->edges * 2 * sizeof(PS_SIT)));
        }
        /* Allocate memory for edge markers if necessary. */
        if (!b->nobound && (*edgemarkerlist == (PS_SIT *) NULL)) {
            *edgemarkerlist = (PS_SIT *) ps_tri_trimalloc((PS_SIT) (m->edges * sizeof(PS_SIT)));
        }
        elist = *edgelist;
...
#ifdef ANSI_DECLARATORS
void ps_tri_writevoronoi(struct mesh *m, struct behavior *b, REAL **vpointlist,
                         REAL **vpointattriblist, PS_SIT **vpointmarkerlist,
                         PS_SIT **vedgelist, PS_SIT **vedgemarkerlist, REAL **vnormlist)
#else /* not ANSI_DECLARATORS */
    void ps_tri_writevoronoi(m, b, vpointlist, vpointattriblist, vpointmarkerlist,
                             vedgelist, vedgemarkerlist, vnormlist)
    struct mesh *m;
    struct behavior *b;
    REAL **vpointlist;
    REAL **vpointattriblist;
    PS_SIT **vpointmarkerlist;
    PS_SIT **vedgelist;
    PS_SIT **vedgemarkerlist;
    REAL **vnormlist;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
    void ps_tri_writevoronoi(struct mesh *m, struct behavior *b, char *vnodefilename,
                             char *vedgefilename, PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
        void ps_tri_writevoronoi(m, b, vnodefilename, vedgefilename, argc, argv)
        struct mesh *m;
        struct behavior *b;
        char *vnodefilename;
        char *vedgefilename;
...

    {
        ...
        if (*vpointlist == (REAL *) NULL) {
            *vpointlist = (REAL *) ps_tri_trimalloc((PS_SIT) (m->triangles.items * 2 *
                                                              sizeof(REAL)));
        }
        /* Allocate memory for Voronoi vertex attributes if necessary. */
        if (*vpointattriblist == (REAL *) NULL) {
            *vpointattriblist = (REAL *) ps_tri_trimalloc((PS_SIT) (m->triangles.items *
                                                                    m->nextras * sizeof(REAL)));
        }
        *vpointmarkerlist = (PS_SIT *) NULL;
        plist = *vpointlist;
        ...
        while (triangleloop.tri != (triangle *) NULL) {
            ...
            dest(triangleloop, tdest);
            apex(triangleloop, tapex);
            ps_tri_findcircumcenter(m, b, torg, tdest, tapex, circumcenter, &xi, &eta, 0);
#ifdef TRILIBRARY
            /* X and y coordinates. */
            ...
            * (PS_SIT *) (triangleloop.tri + 6) = (PS_SIT) vnodenumber;
            triangleloop.tri = ps_tri_triangletraverse(m);
            vnodenumber++;
        }
        ...
        if (*vedgelist == (PS_SIT *) NULL) {
            *vedgelist = (PS_SIT *) ps_tri_trimalloc((PS_SIT) (m->edges * 2 * sizeof(PS_SIT)));
        }
        *vedgemarkerlist = (PS_SIT *) NULL;
        /* Allocate memory for output Voronoi norms if necessary. */
        if (*vnormlist == (REAL *) NULL) {
            *vnormlist = (REAL *) ps_tri_trimalloc((PS_SIT) (m->edges * 2 * sizeof(REAL)));
        }
        elist = *vedgelist;
        normlist = *vnormlist;
        coordindex = 0;
...
#ifdef ANSI_DECLARATORS
void ps_tri_writeneighbors(struct mesh *m, struct behavior *b, PS_SIT **neighborlist)
#else /* not ANSI_DECLARATORS */
    void ps_tri_writeneighbors(m, b, neighborlist)
    struct mesh *m;
    struct behavior *b;
    PS_SIT **neighborlist;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
    void ps_tri_writeneighbors(struct mesh *m, struct behavior *b, char *neighborfilename,
                               PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
        void ps_tri_writeneighbors(m, b, neighborfilename, argc, argv)
        struct mesh *m;
        struct behavior *b;
...

    {
        ...
        struct otri triangleloop, trisym;
        long long elementnumber;
        PS_SIT neighbor1, neighbor2, neighbor3;
        triangle ptr;                         /* Temporary variable used by sym(). */
        ...
        if (*neighborlist == (PS_SIT *) NULL) {
            *neighborlist = (PS_SIT *) ps_tri_trimalloc((PS_SIT) (m->triangles.items * 3 *
                                                                  sizeof(PS_SIT)));
        }
        nlist = *neighborlist;
        index = 0;
        ...
        elementnumber = b->firstnumber;
        while (triangleloop.tri != (triangle *) NULL) {
            * (PS_SIT *) (triangleloop.tri + 6) = (PS_SIT) elementnumber;
            triangleloop.tri = ps_tri_triangletraverse(m);
            elementnumber++;
        }
        * (PS_SIT *) (m->dummytri + 6) = -1;
        ...
        while (triangleloop.tri != (triangle *) NULL) {
            ...
            sym(triangleloop, trisym);
            neighbor1 = * (PS_SIT *) (trisym.tri + 6);
            triangleloop.orient = 2;
            sym(triangleloop, trisym);
            neighbor2 = * (PS_SIT *) (trisym.tri + 6);
            triangleloop.orient = 0;
            sym(triangleloop, trisym);
            neighbor3 = * (PS_SIT *) (trisym.tri + 6);
#ifdef TRILIBRARY
...
#ifdef ANSI_DECLARATORS
void ps_tri_writeoff(struct mesh *m, struct behavior *b, char *offfilename,
                     PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
    void ps_tri_writeoff(m, b, offfilename, argc, argv)
    struct mesh *m;
    struct behavior *b;
    char *offfilename;
    PS_SIT argc;
...
#ifdef ANSI_DECLARATORS
void ps_tri_quality_statistics(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_quality_statistics(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
    ...
    PS_SIT angletable[18];
    PS_SIT aspecttable[16];
    PS_SIT aspectindex;
    PS_SIT tendegree;
    PS_SIT acutebiggest;
    PS_SIT i, ii, j, k;
    ...
    while (triangleloop.tri != (triangle *) NULL) {
        ...
        for (i = 0; i < 3; i++) {
            j = ps_tri_plus1mod3[i];
            k = ps_tri_minus1mod3[i];
            dx[i] = p[j][0] - p[k][0];
            dy[i] = p[j][1] - p[k][1];
        ...
        triarea = ps_tri_counterclockwise(m, b, p[0], p[1], p[2]);
    ...
    printf("  1.1547 - %-6.6g    :  %8lld    | %6.6g - %-6.6g     :  %8lld\n",
           ratiotable[0], (long long)aspecttable[0], ratiotable[7], ratiotable[8],
           (long long)aspecttable[8]);
    for (i = 1; i < 7; i++) {
        printf("  %6.6g - %-6.6g    :  %8lld    | %6.6g - %-6.6g     :  %8lld\n",
               ratiotable[i - 1], ratiotable[i], (long long)aspecttable[i],
               ratiotable[i + 7], ratiotable[i + 8], (long long)aspecttable[i + 8]);
    }
    printf("  %6.6g - %-6.6g    :  %8lld    | %6.6g -            :  %8lld\n",
           ratiotable[6], ratiotable[7], (long long)aspecttable[7], ratiotable[14],
           (long long)aspecttable[15]);
    ...
    for (i = 0; i < 9; i++) {
        printf("    %3lld - %3lld degrees:  %8lld    |    %3lld - %3lld degrees:  %8lld\n",
               (long long)i * 10, (long long)i * 10 + 10, (long long)angletable[i],
               (long long)i * 10 + 90, (long long)i * 10 + 100, (long long)angletable[i + 9]);
    }
    printf("\n");
}
...
#ifdef ANSI_DECLARATORS
void ps_tri_statistics(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    void ps_tri_statistics(m, b)
    struct mesh *m;
    struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void ps_tri_triangulate(char *triswitches, struct triangulateio *in,
                        struct triangulateio *out, struct triangulateio *vorout)
#else /* not ANSI_DECLARATORS */
    void ps_tri_triangulate(triswitches, in, out, vorout)
    char *triswitches;
...
#ifdef ANSI_DECLARATORS
    PS_SIT ps_tri_main(PS_SIT argc, char **argv)
#else /* not ANSI_DECLARATORS */
        PS_SIT ps_tri_main(argc, argv)
        PS_SIT argc;
        char **argv;
#endif /* not ANSI_DECLARATORS */
...

    {
        ...
        ps_tri_triangleinit(&m);
#ifdef TRILIBRARY
        ps_tri_parsecommandline(1, &triswitches, &b);
#else /* not TRILIBRARY */
        ps_tri_parsecommandline(argc, argv, &b);
#endif /* not TRILIBRARY */
        m.steinerleft = b.steiner;

#ifdef TRILIBRARY
        ps_tri_transfernodes(&m, &b, in->pointlist, in->pointattributelist,
                             in->pointmarkerlist, in->numberofpoints,
                             in->numberofpointattributes);
#else /* not TRILIBRARY */
        ps_tri_readnodes(&m, &b, b.innodefilename, b.inpolyfilename, &polyfile);
#endif /* not TRILIBRARY */
...
#ifdef CDT_ONLY
        m.hullsize = ps_tri_delaunay(&m, &b);                /* Triangulate the vertices. */
#else /* not CDT_ONLY */
        if (b.refine) {
            /* Read and reconstruct a mesh. */
#ifdef TRILIBRARY
            m.hullsize = ps_tri_reconstruct(&m, &b, in->trianglelist,
                                            ...
                                            in->numberofsegments);
#else /* not TRILIBRARY */
            m.hullsize = ps_tri_reconstruct(&m, &b, b.inelefilename, b.areafilename,
...
#ifdef TRILIBRARY
                ps_tri_formskeleton(&m, &b, in->segmentlist,
                                    in->segmentmarkerlist, in->numberofsegments);
#else /* not TRILIBRARY */
                ps_tri_formskeleton(&m, &b, polyfile, b.inpolyfilename);
#endif /* not TRILIBRARY */
            }
        }
...
#else /* not TRILIBRARY */
            ps_tri_readholes(&m, &b, polyfile, b.inpolyfilename, &holearray, &m.holes,
                             &regionarray, &m.regions);
#endif /* not TRILIBRARY */
            if (!b.refine) {
                /* Carve out holes and concavities. */
                ps_tri_carveholes(&m, &b, holearray, m.holes, regionarray, m.regions);
            }
        } else {
        ...
        if (b.quality && (m.triangles.items > 0)) {
            ps_tri_enforcequality(&m, &b);           /* Enforce angle and area constraints. */
        }
#endif /* not CDT_ONLY */
        ...
        if (b.order > 1) {
            ps_tri_highorder(&m, &b);       /* Promote elements to higher polynomial order. */
        }
        if (!b.quiet) {
            printf("\n");
        }
        ...
        if (b.nonodewritten || (b.noiterationnum && m.readnodefile)) {
            ...
            ps_tri_numbernodes(&m, &b);         /* We must remember to number the vertices. */
        } else {
            /* writenodes() numbers the vertices too. */
#ifdef TRILIBRARY
            ps_tri_writenodes(&m, &b, &out->pointlist, &out->pointattributelist,
                              &out->pointmarkerlist);
#else /* not TRILIBRARY */
            ps_tri_writenodes(&m, &b, b.outnodefilename, argc, argv);
#endif /* TRILIBRARY */
        }
        if (b.noelewritten) {
            if (!b.quiet) {
...
#ifdef TRILIBRARY
            ps_tri_writeelements(&m, &b, &out->trianglelist, &out->triangleattributelist);
#else /* not TRILIBRARY */
            ps_tri_writeelements(&m, &b, b.outelefilename, argc, argv);
#endif /* not TRILIBRARY */
        }
...
#ifdef TRILIBRARY
                ps_tri_writepoly(&m, &b, &out->segmentlist, &out->segmentmarkerlist);
...
#else /* not TRILIBRARY */
                ps_tri_writepoly(&m, &b, b.outpolyfilename, holearray, m.holes, regionarray,
                                 m.regions, argc, argv);
#endif /* not TRILIBRARY */
            }
        }
...
#ifndef CDT_ONLY
        if (m.regions > 0) {
            ps_tri_trifree((VOID *) regionarray);
        }
#endif /* not CDT_ONLY */
        if (m.holes > 0) {
            ps_tri_trifree((VOID *) holearray);
        }
        if (b.geomview) {
            ps_tri_writeoff(&m, &b, b.offfilename, argc, argv);
        }
#endif /* not TRILIBRARY */
        if (b.edgesout) {
#ifdef TRILIBRARY
            ps_tri_writeedges(&m, &b, &out->edgelist, &out->edgemarkerlist);
#else /* not TRILIBRARY */
            ps_tri_writeedges(&m, &b, b.edgefilename, argc, argv);
#endif /* not TRILIBRARY */
        }
        if (b.voronoi) {
#ifdef TRILIBRARY
            ps_tri_writevoronoi(&m, &b, &vorout->pointlist, &vorout->pointattributelist,
                                &vorout->pointmarkerlist, &vorout->edgelist,
                                &vorout->edgemarkerlist, &vorout->normlist);
#else /* not TRILIBRARY */
            ps_tri_writevoronoi(&m, &b, b.vnodefilename, b.vedgefilename, argc, argv);
#endif /* not TRILIBRARY */
        }
        if (b.neighbors) {
#ifdef TRILIBRARY
            ps_tri_writeneighbors(&m, &b, &out->neighborlist);
#else /* not TRILIBRARY */
            ps_tri_writeneighbors(&m, &b, b.neighborfilename, argc, argv);
#endif /* not TRILIBRARY */
        }
        ...
        if (!b.quiet) {
            ...
            ps_tri_statistics(&m, &b);
        }

#ifndef REDUCED
        if (b.docheck) {
            ps_tri_checkmesh(&m, &b);
            ps_tri_checkdelaunay(&m, &b);
        }
#endif /* not REDUCED */

        ps_tri_triangledeinit(&m, &b);
#ifndef TRILIBRARY
        return 0;
#endif /* not TRILIBRARY */
    }
