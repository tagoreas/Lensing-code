...
void pixsrc_printer::printfitssrcplane(PS_SIT tracker, string basename, string imagename,
                                       ...
                                       pthread_mutex_t *lock, bool writehdr, bool nonstandardname, pthread_mutex_t *fits_lock)
{
    if (fits_lock)
        pthread_mutex_lock(fits_lock);
    ...
    if (fits_lock)
        pthread_mutex_unlock(fits_lock);
}
void pixsrc_printer::printfitsimgplane(PS_SIT tracker, string basename, string imagename,
                                       ...
                                       bool writehdr, bool nonstandardname, pthread_mutex_t *fits_lock)
{
    double *im;
    ...
    printfitssrcplane( tracker, basename, imagename, print2screenname, num,
                       filename,imgx,imgy,im,wcsinfo, lock, writehdr, nonstandardname, fits_lock );
