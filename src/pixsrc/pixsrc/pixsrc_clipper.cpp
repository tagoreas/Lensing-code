...
namespace ClipperLib {
    ...
    static PS_SIT const Unassigned = -1;  //edge not currently 'owning' a solution
    static PS_SIT const Skip = -2;        //edge that would otherwise close a path
...
#define NEAR_ZERO(val) (((val) > -TOLERANCE) && ((val) < TOLERANCE))
    ...
    struct TEdge {
        ...
        PolyType PolyTyp;
        EdgeSide Side;
        PS_SIT WindDelta; //1 or -1 depending on winding direction
        PS_SIT WindCnt;
        PS_SIT WindCnt2; //winding count of the opposite polytype
        PS_SIT OutIdx;
        TEdge *Next;
        TEdge *Prev;
        TEdge *NextInLML;
...
//------------------------------------------------------------------------------
    ...
    PS_SIT PolyTree::Total() const
    {
        PS_SIT result = (PS_SIT)AllNodes.size();
...
//------------------------------------------------------------------------------
    ...
    PS_SIT PolyNode::ChildCount() const
    {
        return (PS_SIT)Childs.size();
    }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
    ...
    {
        PS_SIT size = (PS_SIT)poly.size();
        if (size < 3) return 0;

        double a = 0;
        for (PS_SIT i = 0, j = size -1; i < size; ++i)
        {
...
//------------------------------------------------------------------------------
    ...
    PS_SIT PointInPolygon (const IntPoint &pt, const Path &path)
    {
...
//------------------------------------------------------------------------------
    ...
    PS_SIT PointInPolygon (const IntPoint &pt, OutPt *op)
    {
        //returns 0 if false, +1 if true, -1 if pt ON polygon boundary
        PS_SIT result = 0;
        OutPt* startOp = op;
...
//------------------------------------------------------------------------------
    ...
    bool Poly2ContainsPoly1(OutPt *OutPt1, OutPt *OutPt2)
    {
        ...
        do
        {
            ...
            PS_SIT res = PointInPolygon(op->Pt, OutPt2);
...
//------------------------------------------------------------------------------
    ...
    Clipper::Clipper(PS_SIT initOptions) : ClipperBase() //constructor
    {
        m_ActiveEdges = 0;
        m_SortedEdges = 0;
...
//------------------------------------------------------------------------------
    ...
    void Clipper::IntersectEdges(TEdge *e1, TEdge *e2, IntPoint &Pt)
    {
        ...
        if ( e1->PolyTyp == e2->PolyTyp )
        {
            ...
            {
                PS_SIT oldE1WindCnt = e1->WindCnt;
...
//------------------------------------------------------------------------------
    ...
    OutRec* Clipper::GetOutRec(PS_SIT Idx)
    {
...
//------------------------------------------------------------------------------
    ...
    void Clipper::AppendPolygon(TEdge *e1, TEdge *e2)
    {
        ...
        PS_SIT OKIdx = e1->OutIdx;
        PS_SIT ObsoleteIdx = e2->OutIdx;
...
//------------------------------------------------------------------------------
    ...
    OutRec* Clipper::CreateOutRec()
    {
        ...
        result->Idx = (PS_SIT)m_PolyOuts.size()-1;
        return result;
    }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
    ...
    void Clipper::BuildResult(Paths &polys)
    {
        ...
        for (PolyOutList::size_type i = 0; i < m_PolyOuts.size(); ++i)
        {
            ...
            PS_SIT cnt = PointCount(p);
            if (cnt < 2) continue;
            pg.reserve(cnt);
            for (PS_SIT i = 0; i < cnt; ++i)
            {
                pg.push_back(p->Pt);
                p = p->Prev;
            }
...
//------------------------------------------------------------------------------
    ...
    void Clipper::BuildResult2(PolyTree& polytree)
    {
        ...
        {
            OutRec* outRec = m_PolyOuts[i];
            PS_SIT cnt = PointCount(outRec->Pts);
...
//------------------------------------------------------------------------------
    ...
    {
        for (PS_SIT i = 0; i < m_polyNodes.ChildCount(); ++i)
...
//------------------------------------------------------------------------------
    ...
    {
        PS_SIT highI = (PS_SIT)path.size() - 1;
        if (highI < 0) return;
        ...
        {
            IntPoint ip = m_polyNodes.Childs[(PS_SIT)m_lowest.X]->Contour[(PS_SIT)m_lowest.Y];
...
//------------------------------------------------------------------------------
    ...
    void ClipperOffset::FixOrientations()
    {
        ...
        if (m_lowest.X >= 0 &&
            !Orientation(m_polyNodes.Childs[(PS_SIT)m_lowest.X]->Contour))
        {
            for (PS_SIT i = 0; i < m_polyNodes.ChildCount(); ++i)
            {
    ...
    void ClipperOffset::Execute(PolyTree& solution, double delta)
    {
        ...
        else
        {
            ...
            if (solution.ChildCount() == 1 && solution.Childs[0]->ChildCount() > 0)
            {
                ...
                for (PS_SIT i = 1; i < outerNode->ChildCount(); ++i)
...
//------------------------------------------------------------------------------
    ...
    void ClipperOffset::DoOffset(double delta)
    {
        ...
        for (PS_SIT i = 0; i < m_polyNodes.ChildCount(); i++)
        {
            ...
            PS_SIT len = (PS_SIT)m_srcPoly.size();
            ...
            else
            {
                ...
                k = len - 1;
                for (PS_SIT j = k - 1; j > 0; --j) OffsetPoint(j, k, node.m_jointype);
...
//------------------------------------------------------------------------------
    ...
    void ClipperOffset::OffsetPoint(PS_SIT j, PS_SIT& k, JoinType jointype)
    {
        //cross product ...
...
//------------------------------------------------------------------------------
    ...
    void ClipperOffset::DoSquare(PS_SIT j, PS_SIT k)
    {
...
//------------------------------------------------------------------------------
    ...
    void ClipperOffset::DoMiter(PS_SIT j, PS_SIT k, double r)
    {
        double q = m_delta / r;
...
//------------------------------------------------------------------------------
    ...
    void ClipperOffset::DoRound(PS_SIT j, PS_SIT k)
    {
        double a = std::atan2(m_sinA,
                              m_normals[k].X * m_normals[j].X + m_normals[k].Y * m_normals[j].Y);
        PS_SIT steps = std::max((PS_SIT)Round(m_StepsPerRad * std::fabs(a)), (PS_SIT)1);

        double X = m_normals[k].X, Y = m_normals[k].Y, X2;
        for (PS_SIT i = 0; i < steps; ++i)
        {
            m_destPoly.push_back(IntPoint(
...
//------------------------------------------------------------------------------
    ...
    void AddPolyNodeToPaths(const PolyNode& polynode, NodeType nodetype, Paths& paths)
    {
        ...
        for (PS_SIT i = 0; i < polynode.ChildCount(); ++i)
...
//------------------------------------------------------------------------------
    ...
    void OpenPathsFromPolyTree(PolyTree& polytree, Paths& paths)
    {
        ...
        for (PS_SIT i = 0; i < polytree.ChildCount(); ++i)
